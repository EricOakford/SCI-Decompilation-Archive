...
(class PicView kindof Object
	...
	(method (doit)
		;; Draw this view into the picture via the kernel AddToPic procedure.
		;; Only intended for immediately after a picture is drawn.
		(AddToPic
			view
			loop
			cel
...
;;;		delete
;;;		handleEvent			;give object a chance to respond to an Event
...
;;;	)
	...
	(method (posn newX newY)
		;; Position the View at (newX, newY).  It will appear there on
	...
	(method (delete)
		;; This method is called after each animation cycle if deleteCastMember
		;; is TRUE to purge the cast of dispose:d actors.
	...
	(method (addToPic)
		...
		(self init:, signal:(| signal ADDTOPIC))
		(= deleteCastMember TRUE)
	)


	(method (handleEvent event)
		;; No default behavior in response to events.
	)
...
;;;	)
		...
		(return
			(or
				...
				(and
					...
					; this test causes moveSpeed to react incorrrectly
					(mover triedToMove:)
				)
			)
		)
	...
	(method (distanceTo anObj)
		...
		(return (GetDistance x y (anObj x?) (anObj y?) perspective))
	)
...
;						)
	...
	(method (setDirection dir &tmp nX nY dX dY)
		;; Set an actor's motion to be to a point off the screen in a given
		;; direction.

		;If we're supposed to stop, do so.
		(if (== dir dirStop)
			(self setMotion:0)
			(return)
		)

		;If we're already headed in the requested direction, don't
		;bother with a new mover.
		(if
			(and
				mover
				(== dir
		...
		;Okay, we DO want a new mover.  Make one.
		(= nX x)
		(= nY y)
		(= dX (* xStep 400))
		(= dY (* yStep 400))
