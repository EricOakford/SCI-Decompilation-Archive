...
(instance outsideCompoundRm of Rm
	...
	(method (init)
		...
		(switch prevRoomNum
			...
			(80
				(LoadMany 128 85 684 185 687 287)
				(guardRight
					view: 787
					setLoop: Grooper
					loop: 2
			...
			(else 
				(LoadMany 128 181 185 281 376 684 287)
				(LoadMany 132 82 18 218)
	...
	(method (handleEvent event)
		(cond 
			...
			((Said 'look[<at,around][/room,town,scene]') (Print 81 0))
			((Said 'load/gun') (Print 81 1))
		)
	)
	...
	(method (newRoom newRoomNumber)
		(cond 
			...
			(
				(and
					(not (OneOf newRoomNumber 86 88))
					(!= prevRoomNum 82)
				)
				(ego setScript: stayHereScript)
			)
			(else (super newRoom: newRoomNumber &rest))
		)
	)
)
...
(instance vanInScript of Script
	...
	(method (doit)
		...
		(cond 
			((!= state 4))
			((== local2 1) (= local2 0) (client setScript: orderedOutScript))
		)
	)
	...
	(method (handleEvent event)
		(cond 
			...
			((Said 'open/tray,food') (DontNeedTo))
			((Said 'get,(pick<up)/food,platter,tray')
				(cond 
					((ego has: 13) (Print 81 5))
					((== local0 1) (Print 81 6) (ego get: 13))
					(else (Print 81 7) (ego get: 13))
				)
			)
			(
				(or
			...
			((Said 'look[<at]/compound') (Print 81 10))
			((Said 'address/guard') (Print 81 11))
			((Said 'shoot,fire[/guard,gun]') (Print 81 12))
		)
	)
)
...
(instance outOfVanScript of Script
	...
	(method (doit)
		...
		(cond 
			((!= state 0))
			((and (== (ego onControl: 1) 8) (ego has: 13)) (client setScript: friskScript))
			((or (== (ego onControl: 1) 4) (== local1 1)) (= local1 0) (self cue:))
		)
	)
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((Said 'shoot,fire[/guard,gun]') (client setScript: egoShootScript))
			((Said 'open/tray,food') (DontNeedTo))
			((Said 'get,(pick<up)/food,platter,tray')
				(if (ego has: 13)
					(AlreadyTook)
			...
			((Said 'close/door,truck[<truck]') (DontNeedTo))
		)
	)
)
...
(instance friskScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(cond 
					((== local0 0) (Print 81 19) (Print 81 20))
					((not (tunisia madeCall?)) (Print 81 21) (Print 81 22) (Print 81 19) (Print 81 20))
					(else (Print 81 23))
				)
				(= cycles 1)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(if (< (ego y?) 98)
					(ego setAvoider: Avoid setMotion: MoveTo 194 104 self)
				else
					(= cycles 1)
				)
			)
...
(instance egoShootScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(ego view: 687 loop: 0 cel: 0 setCycle: End self)
				(Print 81 28)
			)
			(3
...
(instance shootOutScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 81 34)
				(ego view: 281 loop: 1 setCycle: End)
...
(instance arabicScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1 (DisposeArab) (= cycles 2))
			(2
				(InitArab ego)
				(= seconds 2)
			)
			(3 (DisposeArab) (= cycles 2))
			(4
				(InitArab guardRight)
				(= seconds 2)
			)
			(5 (DisposeArab) (= cycles 2))
			(6
...
(instance guardLeft of Act
	...
	(method (handleEvent event)
		(cond 
			...
			((Said '[/guard,man]>')
				(cond 
					((Said 'address') (Print 81 41))
					((Said 'look[<at]') (Print 81 42))
				)
			)
		)
	)
		...
		(cond 
			...
			((Said '[/truck]>')
				(cond 
					...
					((Said 'look[<at]') (Print 81 43))
					((Said 'enter,(look<in)') (if (== local3 0) (Print 81 44) else (Print 81 45)))
				)
			)
		)
	)
