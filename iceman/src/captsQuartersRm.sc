...
(instance captsQuartersRm of Rm
	...
	(method (init)
		(super init:)
		(LoadMany 128 30 133 134 230 232 430)
		(HandsOff)
	...
	(method (handleEvent event)
		(cond 
			...
			((Said 'look[<at,around][/room,scene]') (Print 30 0))
			((Said 'address') (Print 30 1))
		)
	)
)
...
(instance waitForEgoScript of Script
	...
	(method (doit)
		(super doit:)
		(if (and local12 (<= state 1)) (self dispose:))
	)
	
	(method (changeState newState)
		(switch (= state newState)
			(0
				(if (== howFast 0) (= seconds 120) else (= seconds 30))
			)
			(1
				(HandsOff)
...
(instance openSafeScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(10
				(if ((inventory at: 2) cel?)
					(= cycles 30)
				else
					(SolvePuzzle subMarine 406 16 1)
					(case setScript: ficheScript)
					(self dispose:)
				)
			)
			(11 (slot setCycle: Beg self))
			(12
				(Print 30 5)
				(User canInput: 1)
				(self init:)
	...
	(method (handleEvent event)
		(cond 
			...
			((Said 'open/briefcase')
				(if (not (case cel?))
					(Print 30 6 #edit @local7 12)
					(cond 
						((not (StrCmp @local7 {762134})) (self cue:))
						((not (StrCmp @local7 {})) (return))
						(else (Print 30 7))
					)
				else
					(Print 30 8)
			...
			((Said 'close/briefcase')
				(if (case cel?)
					(if (not (ego has: 2)) (Print 30 9) (slot dispose:))
					(HandsOff)
			...
			((Said 'look[<at]/top') (if (case cel?) (Print 30 11) else (Print 30 10)))
			((Said 'open/top,compartment') (if (case cel?) (Print 30 12) else (Print 30 10)))
			(
			(Said 'adjust,insert,use/id,card[<id][/slot,hole]')
				(cond 
					((not (case cel?)) (event claimed: 0))
					((ego has: 2)
						(HandsOff)
						(Print 30 13)
						(ego put: 2 curRoom)
			...
			((Said 'get,get/id,card[<id]')
				(cond 
					((ego has: 2) (Print 30 15))
					((IsInvItemInRoom curRoomNum 2) (slot dispose:) (Print 30 16) (ego get: 2) (self init:))
					(else (CantSee))
				)
			)
			((Said 'look,read[<at]/order') (Print 30 17))
		)
	)
)
...
(instance ficheScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4 (slot setCycle: Beg self))
			(5
				(slot dispose:)
				(Print 30 19)
				(ego get: 2)
				(if (IsInvItemInRoom curRoomNum 1)
					(Print 30 20)
					(ego get: 1)
				)
				(HandsOff)
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((Said 'examine,look[<at,in]/briefcase') (Print 30 21))
			((Said 'use/viewer') (if (ego has: 1) (Print 30 22) else (Print 30 23)))
			((Said 'insert/film')
				(cond 
					((ego has: 1)
						(Print 30 24)
					...
					((IsInvItemInRoom curRoomNum 1) (Print 30 26))
					(else (Print 30 27))
				)
			)
			(
...
(instance doc of Prop
	...
	(method (doit)
		...
		(cond 
			((!= local6 0) (-- local6))
			((not (self cel?)) (self setCycle: End) (= local6 (Random 100 300)))
			(else (self setCycle: Beg) (= local6 (Random 100 300)))
		)
	)
	...
	(method (handleEvent event)
		(cond 
			...
			((Said '[/chair]>')
				(cond 
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]') (Print 30 36))
					((Said 'sit') (Print 30 37))
					((Said 'stand') (Print 30 38))
				)
			)
		)
	)
		...
		(cond 
			...
			((Said '[/safe]>')
				(cond 
					...
					((Said 'look<in') (if (not cel) (Print 30 39) else (Print 30 40)))
					((Said 'look[<at]') (Print 30 41))
	...
	(method (handleEvent event)
		(cond 
			...
			((Said '[/door]>')
				(cond 
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]') (Print 30 46))
					((Said 'close') (ItIs))
					((< (ego x?) (bathDoor x?)))
					((GoToIfSaid self event 267 125 'open' 30 42) (= local12 1))
					((Said 'open')
						(= local12 1)
						(HandsOff)
						(self setScript: entryDoorScript)
	...
	(method (handleEvent event)
		(cond 
			...
			(
				...
				(cond 
					((== (case script?) ficheScript) (Print 30 21))
					...
					((not (safeDoor cel?)) (CantSee))
					((not (cast contains: caseView)) (Print 30 21))
					(else
	...
	(method (handleEvent event)
		(cond 
			...
			((Said '[/closet,cabinet]>')
				(cond 
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]') (Print 30 55))
					((Said 'open') (Print 30 56))
					((Said 'close') (ItIs))
				)
			)
		)
...
(instance bunk of RFeature
	...
	(method (handleEvent event)
		(cond 
			...
			((Said 'look[<at][/bunk,bed]') (Print 30 57))
			((Said 'sit') (Print 30 37))
		)
	)
)
	...
	(method (handleEvent event)
		(cond 
			...
			((Said '[/bookcase,shelf]>')
				(cond 
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]') (Print 30 60))
					((Said 'open') (Print 30 61))
					((Said 'close') (ItIs))
				)
			)
		)
