...
(class View of Feature
	...
	(method (setScale param1 &tmp temp0 temp1 temp2)
		(switch param1
			(0
				(&= vm_signal $fffd)
				(= maxScale 0)
...
(class Prop of View
	...
	(method (setScale theObjOrMatchOrY)
		(if (and argc (IsObject theObjOrMatchOrY))
			(if scaler (scaler dispose:))
			(&= vm_signal $fffd)
			((= scaler theObjOrMatchOrY) init: self)
		else
...
(class Actor of Prop
	...
	(method (doit &tmp aState left right txStep tyStep oldXStep oldYStep stepVal)
		...
		(cond 
			...
			(mover
				(mover doit:)
				(if (& vm_signal stopUpdOn)
					(= txStep (/ (* origXStep scaleX) 128))
					(= tyStep (/ (* origXStep scaleY) 128))
					(if (or (!= txStep xStep) (!= tyStep yStep))
						(if (< txStep 1) (= txStep 1))
						(if (< tyStep 1) (= tyStep 1))
						(self setStep: txStep tyStep)
					)
				)
			)
		)
		...
		(if
			(= newLooper
				(cond 
					...
					((not (IsObject theLoop)) (super setLoop: theLoop &rest) 0)
	...
	(method (setStep xs ys)
		...
		(if
		(and (IsObject mover) (mover isMemberOf: MoveTo))
			(mover init:)
		)
	)
	...
	(method (setDirection dir &tmp vx vy xIncr yIncr ang maxCoord pathPts obs)
		...
		(switch dir
			(dirStop (self setMotion: 0) (return))
			(dirN
				(= xIncr (- vx x))
				(= yIncr (- vy y))
			)
