...
(instance rm230 of Room
	...
	(method (init)
		...
		(if playingAsPatti
			...
			(aDoorman init:)
			(Load SOUND 11)
			(if (ego has: iMoney) (Load VIEW 25))
		)
		(NormalEgo)
		(ego init:)
		(cond 
...
(instance aDoor of Prop
	...
	(method (handleEvent event)
		...
		(if (Said 'look/door')
			(if playingAsPatti (Print 230 18) else (Print 230 19))
		)
	)
)
	...
	(method (handleEvent event &tmp item)
		...
		(cond 
			...
			(
				...
				(cond 
					((!= currentStatus egoNORMAL) (GoodIdea))
					((not (& (ego onControl:) cMAGENTA)) (Print 230 24))
					(else
						(Printf 230 25)
						(Format @str 230 26)
						(self changeState: 1)
			...
			(
				...
				(cond 
					((!= currentStatus egoNORMAL) (GoodIdea))
					((not (& (ego onControl:) cMAGENTA)) (Print 230 27))
					(else
						(Print 230 28)
						(Format @str 230 29)
						(self changeState: 1)
			...
			(
				...
				(cond 
					((!= currentStatus egoNORMAL) (GoodIdea))
					((Btst fBeenInChipAndDales) (Print 230 20))
					((not (ego has: iMoney)) (Print 230 30))
					((not (& (ego onControl:) cMAGENTA)) (Print 230 27))
			...
			((Said 'address/man')
				(cond 
					((!= currentStatus egoNORMAL) (GoodIdea))
					((not (& (ego onControl:) cMAGENTA)) (Print 230 31))
					((Btst fBeenInChipAndDales) (Print 230 32))
					(else
						(Print 230 33)
						(Format @str 230 34)
						(self changeState: 1)
			...
			((Said 'give>')
				...
				(event claimed: 0)
				(cond 
					((not (& (ego onControl:) cMAGENTA)) (NotClose))
					((Said '[/!*]') (Print 230 37))
					((not item) (Print 230 38))
					((not (item ownedBy: ego)) (DontHave))
					((== item (inventory at: iPenthouseKey))
						(Print 230 39)
						(Format @str 230 40)
						(self changeState: 1)
					)
					(
						(or
							(== item (inventory at: iPanties))
							(== item (inventory at: iPantyhose))
							(== item (inventory at: iBra))
							(== item (inventory at: iDress))
						)
						(Format @str 230 41)
						(self changeState: 1)
					)
					((== item (inventory at: 18)) (Print 230 42))
					((not (== item (inventory at: 6))) (Print 230 43))
					(else (self changeState: 3))
				)
				(event claimed: TRUE)
			)
