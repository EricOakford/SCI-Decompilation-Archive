...
(instance rm520 of Room
	...
	(method (init)
		...
		(Load SOUND 4)
		(Load SOUND 6)
		(if (ego has: iPantyhose) (Load VIEW 15))
		(super init:)
...
(instance RoomScript of Script
	...
	(method (doit)
		...
		(cond 
			(
			(and (& (ego onControl: origin) cLBLUE) (== currentStatus egoNORMAL)) (self changeState: 1))
			(
			(and (== currentStatus egoDROWNING) (< 8 (++ local0)))
				(= local0 0)
				(ego
			...
			(
			(and (& (ego onControl:) cBLUE) (== currentStatus egoNORMAL)) (self changeState: 12))
		)
	)
	...
	(method (handleEvent event)
		...
		(cond 
			(
				...
				(cond 
					((Btst fDrankRiverWater) (Print 520 0))
