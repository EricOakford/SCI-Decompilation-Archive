...
(instance rm213 of Room
	...
	(method (init)
		(super init:)
		(if (Btst fFlag15) (= newspaperState NSshowroom))
		(if newspaperState (aNewspaper init:))
		(aRiver init:)
		(aRiver2 init:)
		(aTv init:)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				...
				(if (== readingNewspaper TRUE) (= seconds 3))
			)
			(6
				(ego setCel: 0 loop: 1)
				(= seconds (Random 1 4))
		...
		(cond 
			...
			(
				...
				(cond 
					...
					(else (Print 213 18) (event claimed: TRUE))
				)
			)
			((Said 'get/buffet') (Printf 213 19 currentEgo))
			...
			((Said 'look>')
				(cond 
					((Said '/burn') (Printf 213 23 currentEgo))
					((Said '/buffet')
						(cond 
...
(instance aNewspaper of View
	...
	(method (handleEvent event)
		...
		(if (Said 'get')
			(cond 
				(playingAsPatti (Print 213 31))
				(readingNewspaper (AlreadyTook))
				((not (ego inRect: 170 161 221 187)) (NotClose))
				(
				(and (!= currentStatus egoNORMAL) (!= currentStatus egoSITTING)) (GoodIdea))
				(else
					(Ok)
