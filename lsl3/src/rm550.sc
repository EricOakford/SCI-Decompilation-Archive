	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				...
				(Print 550 20)
				(if (>= filthLevel 3) (Print 550 21 #at -1 144))
			)
			(7
				(Print 550 22)
				(User canInput: TRUE)
				...
				(cond 
					((Said '/log')
						(cond 
							...
							((== (ego view?) currentEgoView) (Print 550 15) (Print 550 16 #at -1 144))
							(else (Print 550 13))
						)
					)
...
(instance PattiViewer of Code
	...
	(method (doit)
		...
		(ego brRight: (+ (ego x?) 8))
		(cond 
			((& (ego onControl: origin) cRED) (RoomScript changeState: 14))
			((& (ego onControl:) cGREEN) (ego view: currentEgoView setCycle: Walk))
			((& (ego onControl: origin) cBLUE) (ego view: 552 setCycle: Walk))
			((& (ego onControl: origin) cCYAN) ((Inventory at: iDress) view: 32) (ego view: 551 setCycle: Forward))
		)
	)
)
