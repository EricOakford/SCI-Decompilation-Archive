...
(instance RoomScript of Script
	...
	(method (handleEvent event)
		...
		(cond 
			...
			((Said 'address')
				(cond 
					((and (not haveNoPantyhose) (not (ego has: iPantyhose))) (= haveNoPantyhose TRUE) (PersonSays 335 10))
					((and (not haveNoBra) (not (ego has: iBra))) (= haveNoBra TRUE) (PersonSays 335 11))
					(
						(and
							(not haveNoWine)
							(or (not (ego has: iBottleOfWine)) (!= ((Inventory at: iBottleOfWine) view?) 29))
						)
						(= haveNoWine TRUE)
						(PersonSays 335 12)
					)
