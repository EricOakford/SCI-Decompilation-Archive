...
(class Plane kindof Object
	(properties
		resX					320	; resolution of the plane 
		resY					200	; (for scaling)
...
;;;	)
	...
	(method (findCast theName &tmp node theList)
		...
		(for	((= node (FirstNode (casts elements?))))
				node
				((= node (casts nextNode?)))
