...
(class View of Feature
	...
	(method (init param1 &tmp temp0)
		(= temp0
			(cond 
				...
				((and curRoom (curRoom plane?)) ((curRoom plane?) nSpeed:))
				(else cast)
			)
		)
		(= signal (& signal $fff7))
	...
	(method (nCurPosX theScaleX)
		(self setScale:)
		(if argc (= scaleY (= scaleX theScaleX)))
	)
	
	(method (nCurPosY param1)
		(self setScale:)
		(if argc
...
(class Actor of Prop
	...
	(method (cantBeHere &tmp temp0)
		...
		(= temp0
			(cond 
				((CantBeHere self ((plane nSpeed:) elements?)))
				(
					(and
						(not (& signal $2000))
						curRoom
	...
	(method (oPlane param1 param2)
		(if (== self param1) (param2 cue:) (return))
		(if
			(>
				(GetDistance
					x
	...
	(method (nSaveTime param1 param2)
		(if (== self param1) (param2 cue:) (return))
		(if nYTarget (nYTarget dispose:))
		(= nYTarget (FaceCue new:))
		(nYTarget nStepX: self nStepY: param1 nXTarget: param2)
		(self oPlane: param1 nYTarget)
	)
	
	(method (oCuee param1 param2)
		(if (== self param1) (param2 cue:) (return))
		(self
