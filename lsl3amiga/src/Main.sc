...
(instance LSL3 of Game
	(method (init &tmp startingRoom versionFile)
		...
		;Amiga changes
		(= version {0.001})
		(if (!= (= versionFile (FOpen {version} 1)) -1)
			(FGets version 10 versionFile)
			(FClose versionFile)
		)
		(= volume 15)
	...
	(method (newRoom roomNum theStyle &tmp [str 30])
		(if
			(or
				(and
					(not (OneOf roomNum
		...
		(cond 
			...
			((Said '(drain<out),drain/beer,bottle')
				(cond 
					...
					(else
						(Bottle_of_Wine_ view: 28)
						(Format (Bottle_of_Wine_ name?) 0 22)
						(Print 0 23)
					)
				)
			)
			...
			((or (Said 'give/bill,buck') (Said 'bribe'))
				(cond 
					...
					(else
						(Print 0 55 #icon (A_Twenty_Dollar_Bill view?) 0 0)
						(Print 0 56)
					)
				)
			)
				...
				(cond 
					...
					(else
						(switch (inventory indexOf: i)
							(iOrchids
								(Print 0 65 #icon 11 0 0)
							)
							(iWineBottle
								(switch (Bottle_of_Wine_ view?)
									(28
										(Print 0 66 #icon 28 0 0)
									)
									(29
										(Print 0 67 #icon 29 0 0)
									)
									(else 
										(Print 0 68 #icon 13 0 0)
									)
								)
							)
							(iPanties
								(Print 0 69 #icon 14 0 0)
							)
							(iPantyhose
								(Print 0 70 #icon 15 0 0)
							)
							(iBra
								(Print 0 71 #icon 16 0 0)
							)
							(iDress
								(Print 0 72 #icon 17 0 0)
							)
							(iCoconuts
								(Print 0 73 #icon 19 0 0)
							)
							(else
								(Print 0 64)
								(LogPragFail)
			...
			((Said 'knock')
				(Print 0 76)
				(Print (Format @str 0 77 currentEgo) #at -1 144)
			)
			((Said 'attack')
				(Print 0 62)
			)
