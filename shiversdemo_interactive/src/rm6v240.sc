		...
		(switch (= state newState)
			...
			(2
				(= local2
					(Str
						format:
							{ And the book Egyptian Hieroglyphics Explained is not in its place in the library, but face down here on the office floor. And oddly, one of the pages is__torn out._}
					)
				)
