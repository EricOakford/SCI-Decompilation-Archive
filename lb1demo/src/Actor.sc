...
;;;	)
	...
	(method (setDirection dir &tmp va vx vy xIncr yIncr maxCoord)
		(= vx (curRoom vanishingX?))
		(= vy (curRoom vanishingY?))
		(= va (curRoom vertAngle?))
		(switch dir
			(dirStop
				(self setMotion: 0)
				(return)
			...
			(dirN
				(if va
					...
					(self setMotion: MoveTo (+ x xIncr) -1)
					(return)
				else
					(= xIncr (- vx x))
					(= yIncr (- vy y))
			...
			(dirS
				(if va
					...
					(self setMotion: MoveTo (- x xIncr) 190)
					(return)
				else
					(= xIncr (- x vx))
					(= yIncr (- y vy))
			...
			(else 
				...
				(= xIncr (SinMult maxCoord 100))
				(= yIncr (- (CosMult maxCoord 100)))
			)
		)
