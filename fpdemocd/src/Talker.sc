...
;;;;		Narrator
;;;;		Talker
;EO: THis script was heavily altered for Freddy Pharkas.
...
;;;	)
	...
	(method (say theBuf whoCares noun verb case seq theTalker)
		;
      ...
      (if (not initialized)
		...
		(if (& msgType CD_MSG)
			(if (DoAudio WPlay noun verb case seq theTalker)
				(self startAudio: noun verb case seq theTalker)
			else
				(self startText: theBuf)
			)
		)
	...
	(method (startText theBuf &tmp strLength)
		...
		(= ticks
			(Max 60 (* (/ textSpeed 4) (= strLength (StrLen theBuf))))
		)
...
;;;	)
	...
	(method (show &tmp pnv)
		...
		(if (not underBits)
			(= local0 (Graph GSaveBits nsTop nsLeft nsBottom nsRight PMAP))
...
;;;			(= saveCursor 0)
	...
	(method (startAudio m n v c s &tmp temp0)
