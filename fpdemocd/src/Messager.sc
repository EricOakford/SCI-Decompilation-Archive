...
;;;		(= caller 0)
	...
	(method (sayNext &tmp aTalker [theBuf 200] [msgkey 10])
		;
		; Find the talker and the message and introduce the two of them

		(if firstMessage
			(= aTalker (Message MsgGet theMod theNoun theVerb theCase curSequence @theBuf))
			(= firstMessage 0)
		else
		...
		(if (and	aTalker
			...
			(if (!= aTalker -1)
				...
				(if (& msgType CD_MSG)
					(aTalker
						modNum: theMod
						say: @theBuf self [msgkey 0] [msgkey 1] [msgkey 2] [msgkey 3] [msgkey 4]
					)
				else
					(aTalker
						modNum: theMod
						say: @theBuf self theMod theNoun theVerb theCase curSequence
					)
				)
