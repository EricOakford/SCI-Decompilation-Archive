...
;;;	)
	...
	(method (setDirection dir &tmp vx vy xIncr yIncr ang maxCoord pathPts obs xNode yNode)
		;
		...
		(if (and (= obs (curRoom obstacles?)) useObstacles)
			...
			;EO: Additional code added from Freddy CD demo and SCI32 source
			(= xNode (WordAt pathPts 2))
			(= yNode (WordAt pathPts 3))
			(= xIncr (- xNode x))
			(= yIncr (- yNode y))
			...
			(if altPolyList
				...
				(= pathPts
					(AvoidPath x y xNode yNode (altPolyList elements?) (altPolyList size?) FALSE)
				)
				; If the first node is different from our destination, then
				;	altPolys are involved				
				(if
					(or
						(!= (WordAt pathPts 2) (+ x xIncr))
						(!= (WordAt pathPts 3) (+ y yIncr))
					)
