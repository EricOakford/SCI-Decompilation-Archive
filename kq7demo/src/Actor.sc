	...
	(method (onMe theObjOrX theY &tmp oX oY)
		...
		(return
			(cond
				...
				((not onMeCheck)
				  	(IsOnMe oX oY self (& signal skipCheck))
				)
				(else
					(super onMe: oX oY)
				)
...
;;;	)
	...
	(method (doit &tmp temp0 left right txStep tyStep temp5 temp6 stepVal)
		;
		; This is the method called in every animation cycle
		...
		(if (& signal canUpdate)
			...
			(if mover
				(if (and (& scaleSignal scalable) (not (& scaleSignal noStepScale)))
					(= temp5 (>> origStep $0008))
					(= temp6 (& origStep $00ff))
					(= txStep
						(if (= stepVal (>> (+ (* temp5 scaleX) 64) $0007))
						else
							1
						)
					)
					(= tyStep
						(if (= stepVal (>> (+ (* temp6 scaleY) 64) $0007))
						else
							1
						)
					)
