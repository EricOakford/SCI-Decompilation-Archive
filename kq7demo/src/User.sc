...
;;;	)
	...
	(method (handleEvent event &tmp eType eMsg eMod dir aObj saveCurX saveCurY)
		...
		(if
			(and
				...
				input
				(not eType)
				(or (!= mouseX saveCurX) (!= mouseY saveCurY))
			)
			(OnMeAndLowY init:)
			(cond 
				((thePlane onMe: event)
					(event localize: thePlane)
					(hotCast eachElementDo: #perform OnMeAndLowY event)
				)
				(interfaceHotCast
					(event localize: theInterfacePlane)
					(interfaceHotCast
			...
			(cond 
				(
					(and
						...
						(aObj scratch?)
						(&
							((aObj scratch?) _respondVerbs?)
							(respondCode doit: message)
						)
					)
				...
				(
					...
					(if useHotspots
						(if (not curInvItem)
							(= message dftMessage)
							(theCursor view: 999 loop: 0 setCel: 0)
						else
...
;;;		(= eMsg	(event message?))
		...
		(if
			...
			(if (== message 10)
				(event type: (| userEvent walkEvent))
			else
			...
			(= eMsg (event message?))
			(event localize: theInterfacePlane)
			(if (theInterfaceCast handleEvent: event)
				(return TRUE)
			)
			(event localize: thePlane)
			(cond
