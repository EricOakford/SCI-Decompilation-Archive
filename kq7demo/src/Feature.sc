		...
		(switch (= state newState)
			...
			(3
				...
				(if (not
					...
					;KQ7-specific change
					(if (and (== (client plane?) thePlane) (!= curRoomNum 20))
						(if (== client theExitFeature)
...
;;;	)
	...
	;KQ7-specific methods
	(method (setHotspot args)
		(cond 
			((and (== argc 1) (not [args 0]))
				(if scratch
					(self deleteHotspot:)
				)
			)
			((== argc 1)
				(scratch setProps: [args 0])
			)
			(else
				(if scratch
					(self deleteHotspot:)
				)
				(= scratch ((ScriptID 17) new:))
				(cond 
					((or (not plane) (== plane thePlane))
						(hotCast add: self)
					)
					(interfaceHotCast
						(interfaceHotCast add: self)
					)
				)
				(scratch setProps: [args 0] &rest)
			)
		)
	)
	...
	(method (deleteHotspot)
		...
		(cond 
			...
			(interfaceHotCast
				(interfaceHotCast delete: self)
			)
		)
		(if (== self theExitFeature)
			(cond 
				((not curInvItem)
					(user message: (user dftMessage?))
					(if (and (user canInput:) (user useHotspots?))
						(theCursor view: ARROW_CURSOR loop: 0 setCel: 0)
					)
				)
				((and (user canInput:) (user useHotspots?))
					(theCursor setCel: (- (theCursor cel?) 1))
				)
			)
			(= theExitFeature 0)
		)
	...
	(method (addRespondVerb theVerb)
		(if scratch
			(scratch addRespondVerb: theVerb)
		else
			(Prints {Error-no hotspot for obj.})
		)
	)
