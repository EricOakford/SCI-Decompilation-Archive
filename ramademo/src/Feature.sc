...
(class Feature of Object
	...
	(method (init param1)
		...
		(if plane
			...
			(MonoOut
				{Feature added to unsupported List: %s}
				(self name?)
			)
			(features add: self)
		)
			...
			((not (event type?))
				(if
					(and
						...
						(or
							(and
								(theCursor isInvCursor:)
								(scratch respondsTo: #inRight)
							)
							(scratch respondsTo: (theCursor verb?))
						)
					)
					(self doVerb: 1)
	...
	(method (setHotspot param1)
		(if (not argc)
			(if scratch (self deleteHotspot:))
		else
			(if scratch (self deleteHotspot:))
			(= scratch ((ScriptID 80 0) new:))
			(scratch respondVerbs: [param1 0] &rest)
			(if plane
				(plane addExtMH: self)
			else
				(extMouseHandler add: self)
