...
(class Swat of Game
	...
	(method (restore theSave &tmp num oldCur buf1 buf2 str aPlane aList obj bit node1 node2 node3)
		...
		(if
			...
			(= node1 (KList LNextNode (planes elements?)))
			(while node1
				(planes nextNode: (KList LNextNode node1))
				(= aPlane (KList LNodeValue node1))
				(= node2 (KList LFirstNode ((aPlane casts?) elements?)))
				(while node2
					...
					(while node3
						...
						(if
							(= bit
								(& ((= obj (KList LNodeValue node3)) -info-?) IN_SILIST)
							)
			...
			(if (SaveGame 0 SGCheckSaveGame name num (KString StrGetData version))
				(self getDisc: (CD GetSaveCD))
				(SaveGame 0 SGRestore name num version)
			else
				(= node1 (KList LFirstNode (planes elements?)))
				(while node1
					(planes nextNode: (KList LNextNode node1))
	...
	(method (save &tmp comment num oldCur buf1 buf2 str temp6)
		...
		(switch num
			...
			(else 
				...
				(if
					(not
						(SaveGame 0 SGSave name num (comment data?) (KString StrGetData version))
					)
					(= buf1 (String new:))
					(= buf2 (String new:))
