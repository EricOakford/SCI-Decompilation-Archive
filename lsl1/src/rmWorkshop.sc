...
(instance rm190 of LLRoom
	...
	(method (init)
		(LoadMany VIEW 190 191 192)
		(LoadMany SOUND 195 198 197 196 191 192 199 193 194)
		(theGame setCursor: theCursor FALSE 333 199)
		(typist1 init: cycleSpeed: (+ 2 howFast) setCycle: Forward)
		(typist2 init: cycleSpeed: (+ 3 howFast) setCycle: Forward)
		(smallScreen
			init:
			cycleSpeed: (+ 4 howFast)
			setCycle: Forward
		)
		(nudeLady init: cycleSpeed: (+ 3 howFast) setCycle: Oscillate)
		(theGame setSpeed: 3)
		(blenderLid cycleSpeed: howFast init: stopUpd:)
		(shaft1 cycleSpeed: howFast init:)
		(platform1 cycleSpeed: howFast init:)
		(shaft2 cycleSpeed: howFast init: stopUpd:)
		(platform2 cycleSpeed: howFast init: stopUpd:)
		(deadLarry cycleSpeed: howFast init:)
...
(instance sRecycle of Script
	...
	(method (doit)
		(super doit:)
		(if (== (platform2 cel?) 3) (deadLarry setPri: 1))
		(if
		(and (== state 18) (== (sfxNewLarry prevSignal?) -1))
			(= cycles 1)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(platform1 stopUpd:)
				(deadLarry stopUpd:)
				(blenderLid cycleSpeed: 4 setCycle: EndLoop self)
			)
			(2
				(sfxDumpEgo play:)
				(platform1 setCel: 1)
			...
			(5
				(deadLarry setCel: 4 posn: 60 64 setCycle: EndLoop self)
			)
			(6
				(platform1 cycleSpeed: 3 setCycle: BegLoop self)
			)
			(7
				(platform1 stopUpd:)
				(blenderLid cycleSpeed: 3 setCycle: BegLoop self)
				(sfxLidCloses play:)
			)
			(8
				(blenderLid stopUpd:)
