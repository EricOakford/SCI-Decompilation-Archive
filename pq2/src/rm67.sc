		...
		(switch currentCar
			(13
				(cond 
					...
					((== workCarLocked 1) (Print 67 68) (return 0))
					(workCarTrunkOpened (Print 67 69) (return 0))
					((ego has: 10) (Print 67 70) (return 0))
					(else (curRoom setScript: startEnter) (return 1))
				)
			)
			(33
				(cond 
					...
					((== personalCarLocked 1) (Print 67 68))
					(else (curRoom setScript: startEnter))
				)
			)
		)
	)
...
(instance rm67 of Rm
	...
	(method (init)
		...
		(if (== gamePhase 3)
			...
			(if (Btst 112) (haines stopUpd: x: -20 y: 500))
			(if (Btst 35)
		...
		(if (== prevRoomNum 68)
			(ego posn: 230 200 setPri: -1 init: ignoreActors: 0)
		)
	...
	(method (doit)
		(if (not script)
			(cond 
				((and (== prevRoomNum 68) (not (Btst 35))) (self setScript: afterSearch))
				(
				(and (== global132 (not local0)) (== (not local0) 1)) (= global132 0) (localproc_1bf2))
				((and (== gamePhase 3) (not (Btst 112))) (Bset 112) (self setScript: firstArrived))
			)
			(if
				(and
					(not local0)
				...
				(cond 
					...
					((>= (ego x?) 320) (ego setMotion: MoveTo 310 (ego y?)))
					((>= (ego y?) 230) (ego setMotion: MoveTo (ego x?) 220))
				)
			)
		)
		(if (<= (ego y?) 90)
	...
	(method (handleEvent event)
		...
		(cond 
			((Said 'chat/broad')
				(cond 
					((not (== gamePhase 3)) (event claimed: 0))
					((Btst 35) (Print 67 2))
					((== script afterSearch) (Print 67 3 #at -1 20))
					(else (event claimed: 0))
				)
			)
			((Said 'look/broad')
				(if (or (== script afterSearch) (Btst 35))
					(Print 67 4)
				else
					(event claimed: 0)
				)
			...
			(
				...
				(cond 
					((or (not (== gamePhase 3)) (not (Btst 35))) (event claimed: 0))
					((== script afterSearch) (Print 67 5 #at -1 20))
					(else (Print 67 2))
				)
			)
			(
				(or
					...
					(Said
						'ask,interrogate,get/broad,witness[/number,badge,name,address]'
					)
				)
				(cond 
					((not (== gamePhase 3)) (event claimed: 0))
					((!= script afterSearch) (Print 67 2))
					((not (Btst 51))
					...
					(else (Print 67 7 #at -1 20))
				)
			)
			((Said 'chat/cop,calvin')
				(cond 
					((or (< gamePhase 3) (> gamePhase 3)) (Print 67 8))
					((== script afterSearch) (Print 67 9))
					((Btst 35) (Print 67 10))
					((== gamePhase 3) (Print 67 11))
					(else (Print 67 12 #at -1 40 #width 280))
				)
			)
			((Said 'look/cop,calvin')
				(cond 
					((or (< gamePhase 3) (> gamePhase 3)) (Print 67 8))
					((== script afterSearch) (Print 67 13 #at -1 20))
					((Btst 35) (Print 67 10))
					((== gamePhase 3) (Print 67 11))
					(else (Print 67 14))
				)
			...
			((Said 'look,frisk/trunk')
				(cond 
					((!= currentCar 13) (Print 67 15))
					((not workCarTrunkOpened) (Print 67 16))
					(
						(not
							(ego
			...
			((Said 'close/trunk')
				(cond 
					((not workCarTrunkOpened) (Print 67 20))
					(
						(not
							(ego
								inRect: [trunkRect 0] [trunkRect 1] [trunkRect 2] [trunkRect 3]
			...
			((Said 'deposit,place/briefcase[/trunk]')
				(cond 
					((InRoom 10 13) (Print 67 21))
					((not (ego has: 10)) (Print 67 22))
					((!= currentCar 13) (Print 67 15))
					(
						(not
							(ego
			...
			((Said 'get,remove/briefcase[/trunk]')
				(cond 
					((ego has: 10) (Print 67 25))
					((not (InRoom 10 13)) (Print 67 26))
					(
						(not
							(ego
								inRect: [trunkRect 0] [trunkRect 1] [trunkRect 2] [trunkRect 3]
					...
					(workCarTrunkOpened (Print 67 23 (ego get: 10)))
					(script (Print 67 24))
					(else (self setScript: kitToTrunk) (ego get: 10))
				)
			)
			((Said 'look>')
				(cond 
					((Said '[<at,around][/!*,building,mall,chamber]') (Print 67 27))
					((Said '/tree,dirt,bush') (Print 67 28))
					((Said '/pane')
						(cond 
							(
								(and
					...
					((Said '/lot') (Print 67 34))
					((Said '/sign') (Print 67 35))
					((Said '/crowd') (Print 67 36))
					((Said '/auto,auto')
			...
			((or (Said 'enter/auto') (Said 'open/door'))
				(cond 
					((ego inRect: 224 191 246 210) (self newRoom: 68))
					(
						(ego
					...
					(
					(and (== (ego loop?) 2) (ego inRect: 220 170 240 180)) (Print 67 52))
					(
					(and (== (ego loop?) 3) (ego inRect: 220 170 240 180)) (Print 67 53))
					((ego inRect: 220 170 240 180) (Print 67 54))
					(else (localproc_1bf2))
				)
			)
			((Said 'exit/auto') (if script (Print 67 24) else (self setScript: exitCar)))
			((Said 'unlock/door')
				(cond 
					(
						(ego
					...
					((ego inRect: 16 148 40 152) (Print 67 59))
					(else (Print 67 60))
				)
			)
			((Said 'lock/door')
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(ego
					posn: (if (== currentCar 13) 12 else 28) 160
					loop: 0
					setPri: 12
					setStep: 3 2
					setCycle: Walk
...
(instance startEnter of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(ego setPri: 12 stopUpd:)
				(cond 
					((not keith) (self changeState: 4))
					((not (keith inRect: -10 0 330 240))
						(keith posn: 330 125 setMotion: MoveTo 128 112 self)
						(Print 67 64)
					)
					(else
						(Print 67 65)
			...
			(1
				(keith setPri: 10 setMotion: MoveTo (keith x?) 140 self)
			)
			(2
				(keith setPri: 10 setMotion: MoveTo 29 145 self)
			)
			(3
				(ego setPri: 12)
				(if (== currentCar 13) (keith setMotion: MoveTo 30 140))
				(carDoor
					view: 51
					loop: (if (== currentCar 13) 2 else 0)
					cel: 0
			...
			(4
				(if (== currentCar 13)
					(keith setLoop: 2)
					(if
					(and (not (Btst 156)) (or (Btst 50) (Btst 51)))
						(Bset 156)
						(Print 67 66)
					)
				)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(if keith
					(Print 67 75 #at -1 40)
					(keith illegalBits: -32768 setMotion: MoveTo 155 125 self)
				else
					(= seconds 3)
				)
			)
