...
(procedure (ShakePlane  aPlane 		; the plane to shake
	...
	; time to rock 'n roll
	(for (TRUE) n ((-- n))
		(aPlane setRect: l1 t1 r1 b1 TRUE)
		(FrameOut)
		(aPlane setRect: l2 t2 r2 b2 TRUE)
		(FrameOut)
	)

	; put plane back to normal
	(aPlane setRect: l t r b TRUE)
)
...
;;;	)
	...
	(method (doit aPlane aStyle time aBack ani &tmp xDir yDir)
		...
		(cond
			...
			(divisions
				(SetShowStyle style plane seconds back priority animate refFrame blackScreen (pFadeArray data?) divisions) 
			)
			(else
				(SetShowStyle style plane seconds back priority animate refFrame blackScreen (pFadeArray data?))
			)
		)
	)
)
