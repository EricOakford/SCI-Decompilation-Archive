...
;;;	)
	...
	(method (play newVol &tmp argCount)
		...
		;EO: Condition commented out to prevent glitched sounds
	...
	(method (mute value channel &tmp i theVal)
		...
		(if (< argc 2)
			(for	((= i 1))
					(< i 17)
					((++ i))
				(DoSound MidiSend self i mCONTROLLER mMUTE theVal)
			)
		else
			(DoSound MidiSend self channel mCONTROLLER mMUTE theVal)
		)
	)
