...
(procedure (localproc_06fa)
	...
	(egoTell init: ego @local158 @local166)
	(manuTell
		init: (ScriptID 41 1) @local168 @local175 @local181
	)
...
(instance rm730 of Rm
	...
	(method (init)
		...
		(= [local175 4] @local196)
		(super init:)
		(globalSound number: 915 setLoop: -1 play: 127)
		(leafBed init:)
		(ropeBridge init:)
		(nest init:)
		(notherBridge init:)
...
(instance egoTell of Teller
	...
	(method (showDialog)
		(super
			showDialog:
				-20
				(if (== local201 1) (== local200 0) else 0)
				-21
				(== local200 1)
				-22
				(== local200 2)
	...
	(method (doChild)
		(return
			(switch query
				(-20
					(if (<= (ego trySkill: 13 250) 0)
						(++ local200)
					else
						(curRoom setScript: theyLeave)
					)
				)
				(-21
					(if (<= (ego trySkill: 13 200) 0)
						(++ local200)
					else
						(curRoom setScript: theyLeave)
					)
...
(instance manuTell of Teller
	...
	(method (showDialog)
		(super showDialog: -11 (= local201 1))
	)
	...
	(method (doChild)
		(return
			(switch query
				...
				(-13 (super doChild: query))
				(-28 (super doChild: query))
				(-15
					(= local201 1)
					(return query)
				)
