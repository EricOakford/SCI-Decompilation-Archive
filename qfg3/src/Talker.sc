...
;;;	)
	...
	(method (say theBuf v c s whoCares m)
		;
	...
	(method (startText theBuf &tmp [str 1000] strLength)
	...
	(method (doit)
		...
		(if (!= ticks -1)
			(if (and	(> (- gameTime ticks) 0)
				...
				(and
					(if cDAudio (== (DoAudio Loc) -1) else 1)
					(not keepWindow)
				)
