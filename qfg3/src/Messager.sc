	...
	(method (say args &tmp aNoun aVerb aCase aMod [buffer 20] i)
		...
		(if (== aNoun NEXT)
			...
			(if (or (and	(& msgType TEXT_MSG)
				...
				(Print
					addTextF:	{<Messager> %d: %d, %d, %d, %d not found}
	...
	(method (sayNext theMod theNoun theVerb theCase theSeq
		...
		(if (and (= aTalker (Message MsgNext 0))
			...
			(if (!= aTalker -1)
				...
				(if argc
					(aTalker
						say: theNoun theVerb theCase theSeq self theMod
					)
				else
					(aTalker
						say: NULL NULL NULL NULL self
					)
				)
