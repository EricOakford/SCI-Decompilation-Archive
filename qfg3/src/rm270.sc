...
(instance rm270 of Rm
	...
	(method (doit &tmp temp0)
		...
		(cond 
			(script)
			((not (if (< 5 (ego x?)) (< (ego x?) 315))) (HandsOff) (self setScript: sExit))
			((== temp0 2) (HandsOff) (self setScript: toRestLeft))
			((== temp0 4) (HandsOff) (self setScript: toRestRight))
			((== temp0 8) (HandsOff) (self setScript: toApoth))
			((== temp0 16) (HandsOff) (self setScript: toKreesha))
			((== temp0 32)
				(if (not (& (ego signal?) $0010))
					(ego signal: (| (ego signal?) $0010) priority: 9)
				)
			)
			((& (ego signal?) $0010) (ego signal: (& (ego signal?) $ffef)))
		)
	)
...
(instance walkAround of Script
	...
	(method (changeState newState &tmp temp0 temp1)
		(switch (= state newState)
			(0
				(if
				(and (< local208 local206) (not (Random 0 2)))
					(++ local208)
			...
			(2
				(if (> register 9) (= register (- register 10)))
				(= [local196 register] 0)
				(client
					x: (- 0 (Random 200 500))
					y: (- 0 (Random 200 500))
...
(instance moveOut of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(ego setMotion: PolyPath local219 local220 self)
				(= seconds 3)
			)
			(1
				(if
				(and (!= (ego x?) local219) (!= (ego y?) local220))
					(self init:)
				)
				(HandsOn)
				(self dispose:)
