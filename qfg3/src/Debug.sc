		...
		(switch (event type?)
			(keyDown
				...
				(switch (event message?)
					...
					(`@b
						(Printf @str
							{You have %d Royals, %d Commons, and %d Dinars.}
							((inventory at: iRoyals) amount?)
							commons
							numDinars
						)
						((inventory at: iRoyals) amount: (GetNumber {Enter Gold:}))
						(= commons (GetNumber {Enter Commons:}))
						(= numDinars (GetNumber {Enter Dinars:}))
					)
					(`@k
