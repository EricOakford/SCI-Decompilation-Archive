	...
	(method (handleEvent event &tmp temp0 temp1 i)
		(cond 
			...
			((and (== (event type?) saidEvent) global212)
				(= local0 -1)
				(= i [local217 (- global212 1)])
				(while (< i [local217 global212])
					(if (== global211 [local145 i])
						(= local0 i)
						(break)
					)
					(++ i)
				...
				(if (!= local0 -1)
					(= temp1 (<< local0 $0001))
					(if (and (== local0 42) (& deadGuests $0001))
