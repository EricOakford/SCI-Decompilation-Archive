	...
	(method (handleEvent event &tmp temp0 temp1 i)
		(cond 
			...
			((and (== (event type?) saidEvent) global212)
				(= local0 -1)
				(= i [local269 (- global212 1)])
				(while (< i [local269 global212])
					(if (== global211 [local179 i])
						(= local0 i)
						(break)
					)
					(++ i)
