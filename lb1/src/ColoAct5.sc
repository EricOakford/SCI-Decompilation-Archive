	...
	(method (handleEvent event &tmp temp0 temp1 i temp3)
		(cond 
			...
			((and (== (event type?) saidEvent) global212)
				(= local0 -1)
				(= i [local286 (- global212 1)])
				(while (< i [local286 global212])
					(if (== global211 [local193 i])
						(= local0 i)
						(break)
					)
					(++ i)
				...
				(if (!= local0 -1)
					(= temp1 (<< local0 $0001))
					(cond 
						((and (== local0 51) (& deadGuests $0001)) (= temp3 92))
						((and (== local0 64) (& deadGuests $0002)) (= temp3 93))
						((and (== local0 52) (& deadGuests $0004)) (= temp3 95))
						((and (== local0 49) (& deadGuests $0008)) (= temp3 94))
						(else (= temp3 0))
					)
					(if temp3
