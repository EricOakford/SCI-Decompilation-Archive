...
(instance Room14 of Rm
	...
	(method (init)
		...
		(= east 10)
		(super init:)
		(LoadMany 128 1 25 56)
		(LoadMany 132 41 43 44 47 48 97)
		(if (not global206) (Load rsVIEW 47))
		(Chair
			setPri: 10
			ignoreActors: 1
	...
	(method (doit)
		(if (FirstEntry) (Print 14 0))
		(if (& (ego onControl: 0) $0010) (curRoom newRoom: 3))
		(if (& (ego onControl: 0) $0008) (curRoom newRoom: 9))
		(if
			(and
		...
		(return
			(if (== (event type?) evSAID)
				(cond 
					((Said 'examine>')
						(cond 
							((Said '[<around,at][/room]') (Print 14 0))
							((Said '/path,(boulder<stepping)') (Print 14 1))
						)
					)
					((Said 'get,get,chop,untie,examine/hemp') (Print 14 2))
				)
			else
				0
			)
	...
	(method (newRoom n)
		(if (== n 63)
			(cSound stop:)
			(ego illegalBits: -32768 setPri: -1 setLoop: -1)
		)
		(super newRoom: n)
	)
)
...
(instance comeIn of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(if (== currentAct 6) (Print 14 4) else (Print 14 5))
				(= cycles 1)
			)
			(4
				(= local2 1)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				...
				(ego loop: 1 cel: 0 cycleSpeed: 0 setCycle: End self)
				(Chair setCycle: End)
			)
			(4
				(cls)
...
(instance Squirel of Prop
	...
	(method (handleEvent event)
		(cond 
			((Said 'get,capture/squirrel') (Print 14 9))
			(
			(or (MousedOn self event 3) (Said 'examine/squirrel')) (event claimed: 1) (Print 14 10))
		)
	)
)
	...
	(method (handleEvent event)
		(cond 
			...
			(
				...
				(cond 
					(global206 (Print 14 12))
					((ego inRect: 27 140 77 167) (ego setScript: swinging))
					(else (NotClose))
				)
			)
			((Said 'get/swing') (Print 14 13))
			(
			(or (MousedOn self event 3) (Said 'examine/swing'))
				(event claimed: 1)
				(if global206 (Print 14 14) else (Print 14 15))
			)
		)
	)
)
...
(instance Door of Prop
	...
	(method (handleEvent event)
		(cond 
			((Said 'break/door') (Print 14 16))
			((Said 'unbar/door') (if (>= currentAct 7) (Print 14 17) else (Print 14 18)))
			((Said 'bang[/door]')
			...
			(
			(or (MousedOn self event 3) (Said 'examine/door')) (event claimed: 1) (Print 14 20))
		)
	)
)
...
(instance Window1 of RFeature
	...
	(method (handleEvent event)
		(cond 
			...
			((Said 'break/window') (Print 14 22))
			((Said 'open/window') (Print 14 23))
			(
			(or (MousedOn self event 3) (Said 'examine/window')) (event claimed: 1) (Print 14 24))
		)
	)
)
...
(instance Tower of RFeature
	...
	(method (handleEvent event)
		(if
		(or (MousedOn self event 3) (Said 'examine/tower'))
			(event claimed: 1)
			(Print 14 25)
		)
	)
...
(instance Chapel of RFeature
	...
	(method (handleEvent event)
		(if
		(or (MousedOn self event 3) (Said 'examine/chapel'))
			(event claimed: 1)
			(Print 14 26)
		)
	)
