...
;		)
	...
	(method (fade newVol fTicks fSteps fEnd &tmp argCount theTheClient)
		(= argCount argc)
		...
		(if (> argCount 1)
			(DoSound FadeSound self theTheClient fTicks fSteps fEnd)
		else
			(DoSound FadeSound self theTheClient 25 10 (not theTheClient))
		)
	)
