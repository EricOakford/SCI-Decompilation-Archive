...
;;;;	September 4, 1991
;;;;
;;;;	This is the base Talker class, with a few modifications for Larry 5
;;;;	by Brian K. Hughes.  Feel free to remove these comments when you put
;;;;	this module back into the system.
;;;;
;;;;
;;;;	Changes for Larry 5 Talkers:
;;;;
;;;;	1) Added the viewInPrint property to indicate whether the talker is to
;;;;		appear as an icon in the print window or not.
;;;;
;;;;	2)	Added the useFrame property to indicate whether the frame or bust
;;;;		should be used for the icon (only important for sizing the print
;;;;		window on viewInPrint talkers).
;;;;
;;;;	3)	The underbits area is determined in a separate method, setSize, and
;;;;		is invoked from the init method only.  This function used to be in
;;;;		the show method.
;;;;
;;;;	4)	The show method, which actually draws the cels on the screen, is now
;;;;		only invoked from the startAudio and startText (if the talker is NOT
;;;;		a viewInPrint talker) methods.  This eliminates the "pre-flash" of
;;;;		the image before the text window appears in viewInPrint talkers.
;;;;
;;;;	5) Changed the time displayed to more accurately reflect the textSpeed
;;;;		slider based on an olde intrinsic algorithm developed empirically
;;;;		by humans who knew how to read.
;;;;
;;;;
;;;;	Classes:
;;;;		RTRandCycle
...
;;;	(methods 
;;;		init 		  		;initialize, add to fastCast, and optionally say
...
;;;	)
	...
	(method (init bustView prop syncedProp modNum charNum msgNum dWD whoCares
		...
		(++ talkersOnScreen)
		(self	setSize:)	; Sets the saveBits without showing talker yet
		(if (>= argc 4)
	...
	(method (startText modNum charNum msgNum &tmp [buffer 500] theLoop theWidth)
		(if viewInPrint
			(= theLoop (if useFrame loop else (bust loop?)))
			;; Draw Talker face and text and leave it for a calculated
			;; amount of time, based on textSpeed slider
			(Format @buffer  modNum charNum)
		;	(= ticks (* (- 11 textSpeed) (StrLen @buffer)))
			;**	This formula must match the TPrint formula in 000.sc:
			(= ticks (Max 180 (/ (* textSpeed (StrLen @buffer)) 2)))
		...
		;	(= ticks (* 5 (StrLen @buffer)))
			...
			(if mouth (mouth setCycle: RTRandCycle ticks))
			(if eyes	 (eyes  setCycle: RTRandCycle ticks))
			(if (> (+ x talkWidth) 318)		; stop at right edge
				(= theWidth (- 318 x))
			else
				(= theWidth talkWidth)
			)
