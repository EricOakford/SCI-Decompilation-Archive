...
(instance rm82Script of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				(Print 82 25 #at -1 20 #draw)
				(User canInput: TRUE)
				(if (not (ego has: iAirsickBag))
					(HandsOff)
					(Print 82 26)
					(Print 82 27 #at -1 130)
					(= currentStatus egoDYING)
				)
			)
			(8
			...
			(11
				...
				(theGame changeScore: 5)
				(Print 82 29 #at -1 20 #draw)
				(Print 82 30 #at -1 130)
				(= bombIsLit TRUE)
				(User canInput: TRUE)
				(= seconds 5)
			)
	...
	(method (handleEvent event)
		...
		(if (and (ego has: iHairRejuvenator) (Said 'crap,drain/rejuvenator'))
			(Print 82 13)
			(Print 82 14 #at -1 130)
			(ego put: iHairRejuvenator -1)
		)
		(if (Said 'conceal,apply,throw/rejuvenator,bottle,bomb')	;bomb spec added for Amiga
			(cond 
		...
		(if (Said 'stick,conceal,jerk,conceal/rejuvenator,bottle/bag')
			(cond 
				((or (not (ego has: iHairRejuvenator)) (not (ego has: iAirsickBag)))
					(DontHave)
				)
				((and (!= currentStatus egoNORMAL) (!= currentStatus egoHOLDINGBOTTLE))
					(Print 82 15)	;Amiga addition
					;Good idea. But now is not the time.
				)
				(bombHasWick
					(Print 82 17)
				)
