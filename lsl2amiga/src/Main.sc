...
(procedure (GetVersion)
	...
	;NOTE: Added for 1.003 (Amiga version)
	(Print
		(Format @str 0 140 version)
		#font smallFont
		#mode teJustCenter
		#title {Leisure Suit Larry II}
		#icon 836 0 0
	)
)
...
(instance LSL2 of Game
	(method (init &tmp temp0)
		...
		(= version {1.003})	;Amiga version; the newest PC version is 1.002
		(Load FONT smallFont)
		(Load FONT userFont)
		(Load FONT bigFont)
	...
	(method (startRoom roomNum &tmp temp0)
		;Amiga addition
		(LoadMany FALSE
			AVOIDER JUMP ORBIT PATH EXTRA TEXTRA DOOR
			RFEATURE GROOPER DEMO NAMEFIND CHASE FOLLOW WANDER
			REVERSE TIMER SORT COUNT DPATH QSCRIPT FORCOUNT
			CAT QSOUND TRACK LOADMANY
		)
