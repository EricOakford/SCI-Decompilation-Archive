...
(instance rm160 of GloryRm
	...
	(method (init)
		(theGame handsOff:)
		(Bclr 6)
		(thePlane back: 112 top: 0 inTop: 0)
		(super init: &rest)
		(textStuff init:)
		(skeleton init:)
		(mouseDownHandler addToFront: sDoCredits)
		(keyDownHandler addToFront: sDoCredits)
		(theIconBar disable:)
		(curRoom setScript: sDoCredits)
	)
	
	(method (dispose)
		(mouseDownHandler delete: sDoCredits)
		(keyDownHandler delete: sDoCredits)
		(thePlane back: 0 top: 10 inTop: 10)
		(theIconBar enable:)
		(super dispose:)
	)
)
...
(instance sDoCredits of Script
	...
	(method (handleEvent event)
		(cond 
			...
			((== (event type?) evVERB) (event claimed: 1))
		)
