...
(instance rm635 of GloryRm
	...
	(method (init)
		...
		(curRoom setScript: rewardCeremony)
		(super init:)
	)
)
...
(instance rewardCeremony of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4 (messager say: 3 6 4 0 self))
			(5 (messager say: 2 6 5 0 self))
			(6 (messager say: 2 6 6 0 self))
			(7
				(innKeeper
					setCycle: 0
					signal: (& (innKeeper signal?) $fffe)
				)
				(shopKeeper
					setCycle: 0
					signal: (& (shopKeeper signal?) $fffe)
				)
				(ego
					view: 161
					scaleX: 128
