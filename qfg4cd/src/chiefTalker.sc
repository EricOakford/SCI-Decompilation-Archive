...
(instance chiefTalker of GloryTalker
	...
	(method (dispose param1)
		(if (or (not argc) param1)
			(if (not (if gTeller (gTeller talker?)))
...
(instance chiefFrame of View
	...
	(method (init)
		(= view (if (Btst 307) 349 else 348))
		(= y (if (Btst 307) 12 else 2))
		(super init: &rest)
	)
)
...
(instance chiefMouth of Prop
	...
	(method (init)
		(= view (if (Btst 307) 349 else 348))
		(= x (if (Btst 307) 38 else 29))
		(= y (if (Btst 307) 77 else 110))
		(super init: &rest)
	)
)
...
(instance chiefEyes of Prop
	...
	(method (init)
		(= view (if (Btst 307) 349 else 348))
		(= x (if (Btst 307) 43 else 38))
		(= y (if (Btst 307) 61 else 93))
		(super init: &rest)
	)
)
