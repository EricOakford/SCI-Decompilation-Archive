...
(instance rm520 of Rm
	...
	(method (init &tmp temp0)
		...
		(theFlo init:)
		(theDroole init:)
		(walkHandler addToFront: theExit)
		(curRoom setRegions: 505)
		(switch global125
			(0
				(theGame handsOff:)
				(LoadMany 128 524 522)
				(curRoom setScript: sWaitress self)
			)
			(1
				(theGame handsOff:)
				(LoadMany 128 524 521)
				(curRoom setScript: sSalesman self)
			)
			(2
				(theGame handsOff:)
				(LoadMany 128 524 523)
				(curRoom setScript: sQuirk self)
			)
			(3
				(theGame handsOn:)
				(LoadMany 128 524)
				(curRoom setScript: sCrew self)
			)
			(else 
				(theGame handsOn:)
				(LoadMany 128 524)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(if (Btst 48) (drink setScript: sMonkeys))
				(= cycles 1)
			)
			(1 (self dispose:))
		)
...
(instance sWaitress of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(theMusic2 number: 503 loop: 1 play:)
...
(instance sMonkeysInDrink of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(theMusic1 number: 25 loop: -1 play:)
...
(instance drink of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(34
				(= local3 1)
				(curRoom setScript: sMonkeysInDrink)
			)
			(else 
		...
		(switch theVerb
			(2
				(switch local4
					(0
						(= local4 1)
						(messager say: 4 2 0 0)
					)
					(else  (messager say: 4 2 1 0))
				)
			)
			(else 
				(super doVerb: theVerb &rest)
		...
		(switch theVerb
			(2
				(switch local5
					(0
						(= local5 1)
						(messager say: 2 2 0 0)
					)
					(else  (messager say: 2 2 1 0))
				)
			)
			(else 
				(super doVerb: theVerb &rest)
