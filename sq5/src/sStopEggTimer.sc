		...
		(switch (= state newState)
			(0
				(if (Btst 84)
					...
					(theMusic2 number: 219 loop: 1 play:)
					(if (>= (eureka puke?) 4) (SolvePuzzle 201 100))
				)
			)
			(1
				(= next sShowEggTimer)
				...
				(if (cast contains: (ScriptID 202 14))
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(2
				...
				(localproc_00fa)
				(theMusic2 number: 220 loop: 1 play:)
				(= ticks 50)
			)
			(3
...
(instance eggTimer of Prop
	...
	(method (init)
		(if (Btst 84) (waddle setScript: sShowEggTimer))
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4
				(cond 
					((InRect 220 121 235 135 mouseX mouseY)
						(if (not (Btst 84))
							(theButton show: 0)
							(waddle setScript: sStartEggTimer)
						)
					)
					((InRect 235 121 248 135 mouseX mouseY)
						(if (not (Btst 84))
							(theButton show: 1)
							(curRoom setScript: sStopEggTimer)
						else
							(messager say: 8 4 0 1 self 202)
						)
					)
