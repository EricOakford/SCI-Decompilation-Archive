	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(BeasHead init: setCycle: Forward)
				(BeasArm init: stopUpd:)
				(BeasEyes init: setCycle: Forward)
				(BeaLegs init: setCycle: Forward)
				(= seconds 3)
			)
			...
			(5
				...
				(RogersEyes init: setCycle: Forward)
...
(instance sDoOids of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(oid2 init: setLoop: (+ (oid2 loop?) 7) setCycle: EndLoop)
				(oid3 init: setLoop: (+ (oid3 loop?) 7) setCycle: EndLoop)
			...
			(4
				(oid1 setCycle: Oscillate 1)
				(puke1Miss init:)
				(theMusic2 number: 519 setLoop: 1 play:)
				(= seconds 1)
			)
			(5
				(oid2 setCycle: Oscillate 1)
				(puke2Miss init:)
				(theMusic2 number: 519 setLoop: 1 play:)
				(= seconds 1)
			)
			(6
				(oid4 setCycle: Oscillate 1)
				(puke4HitBea init:)
				(theMusic2 number: 519 setLoop: 1 play:)
				(= seconds 3)
			)
			(7
				(oid3 setCycle: Oscillate 1)
				(puke3Miss init:)
				(theMusic2 number: 519 setLoop: 1 play:)
				(= seconds 2)
			)
...
(instance puke4HitWall of MyPuke
	...
	(method (cue)
		(if (== loop 6)
			(self setLoop: 5 setCel: 0 posn: 28 41 setCycle: EndLoop self)
		else
			(self dispose:)
		)
	)
