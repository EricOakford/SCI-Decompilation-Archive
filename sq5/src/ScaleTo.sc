...
;;;;
;;;;	Author:	Robert Lindsley & Randy MacNeill
;;;;	Updated:	10/28/92 Robert Lindsley
;;;;
	...
	(method (init who whereToScale theStep theTime whoCares)
		(if argc
			...
			(if (>= argc 2)
				...
				(if (>= argc 3)
					(if (IsObject theStep)		  ;if it's a caller no need to look for any more parameters.
						(= caller theStep)
					else
						(= step theStep)
						(if (>= argc 4)
		...
		(= waitCount (+ (= saveWaitCount waitCount) gameTime))
