...
(instance rm530 of Rm
	...
	(method (init)
		...
		(theCellExit init:)
		(theLock init:)
		(walkHandler addToFront: theCellExit)
		(switch prevRoomNum
			(510
				(curRoom picture: 71 setScript: sInJail1)
			)
			(else 
				(ego get: 8)
				(curRoom picture: 71 setScript: sInJail1)
	...
	(method (dispose)
		(walkHandler delete: theCellExit)
		(DisposeScript 991)
		(super dispose: &rest)
	)
)
...
(instance sSpikeOnCell of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(28
				(theMusic1 number: 36 setLoop: -1 play:)
				(bars dispose:)
				(curRoom newRoom: 510)
			)
		)
...
(instance cTalker of PriorityTalker
	...
	(method (init)
		...
		(if (not (Btst 49)) (self cel: 2 y: 41))
...
(instance theCell of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(31
				(curRoom setScript: sSpikeOnCell)
			)
			(else 
...
(instance theCliffy of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(if (== local0 0)
					(curRoom setScript: sCliffySpeaks)
				else
					(messager say: 4 0 1 0)
				)
			)
			(else 
				(super doVerb: theVerb &rest)
