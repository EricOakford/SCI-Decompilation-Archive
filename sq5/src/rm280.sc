...
(instance rm280 of Rm
	...
	(method (init)
		...
		(LoadMany 128 274 275 276)
		(garbageBag init:)
		(curRoom setScript: sTrashPickup)
		(super init:)
	)
...
(instance sTrashPickup of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(theGame handsOff:)
				(theMusic2 number: 21 loop: -1 play: 0 fade: 127 10 10 0)
				(= seconds 2)
			)
			(1
				(PalVary pvUNINIT)
			...
			(9
				(shipEureka init:)
				(theMusic1 number: 225 loop: -1 play: 40 fade: 127 5 20 0)
				(= cycles 1)
			)
			(10
				(shipEureka
				...
				(if (< (++ register) 15)
					...
					(theMusic1 number: 227 loop: 1 play:)
				)
				(= ticks 4)
			)
			(11
