...
;;;	)
	...
	(method (handleEvent event &tmp eType eMsg eMod dir aObj)
		...
		;     Prints
		;        ³
		;        ÃÄ[direction]ÄÄÄÄ directionHandler
		;        ³                  ÃÄ[walk]Ä ego
		;        ³                  pmouse
		;			³
		;        ÃÄ[keyDown]ÄÄÄÄÄÄ keyDownHandler
		;			³
		;        ÃÄ[mouseDown/Up]Ä mouseDownHandler
		;		   ³
		;     IconBar
		;        ³
		;        ÃÄ[walkEvent]ÄÄÄÄ walkHandler
		;        ³                  ego
		;        ³
		;      Cast ÄÄÄÄÄ¿
		;        ³        ³
		;     Features ÄÄÅÄ (sorted features uses all these with OnMeAndLowY)
		;        ³        ³
		;     AddToPics ÄÙ
		;        ³
		;     Regions
...
;OLD			)
			...
			; Give the event to the prints list next, if one exist
			(if (and prints
						(prints handleEvent: event)
					)
				(return TRUE)
			)
					...
					(cond
						((and	(== eMsg speechNoun)
								(or	(= aObj (cast 			firstTrue: #perform findNoun))
										...
										(= aObj (addToPics	firstTrue: #perform findNoun))
								)
							)
		...
		(if (and	input (& eType userEvent))
			(cond
				...
				(useSortedFeatures
					...
					(addToPics	eachElementDo: #perform: OnMeAndLowY event)
				...
				; We're not using sorted features, so give the addToPics a shot

				((addToPics handleEvent: event)
					(return TRUE)
				)
			)

			; Nobody in the cast, features, or addToPics claimed it, so try
			;	the regions
