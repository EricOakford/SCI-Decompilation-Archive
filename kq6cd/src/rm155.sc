	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(curRoom drawPic: 136 100)
				(SetCursor 0)
				(mane1 init: setCycle: Forward)
				(mane2 init: setCycle: Forward)
				(mane3 init: setCycle: Forward)
				(mane4 init: setCycle: Forward)
				(mane5 init: setCycle: Forward)
				(mane6 init: setCycle: Forward)
			...
			(6
				(isle1 setCycle: EndLoop setMotion: MoveTo 62 172 self)
				(isle2 setCycle: EndLoop setMotion: MoveTo 256 177 self)
			)
			(7
				(PalVary pvUNINIT)
				(if (== prevRoomNum 340)
