...
(procedure (SortedAdd &tmp newEventHandler newEventHandler_2 newEventHandler_3)
	((= frontList (EventHandler new:)) add: name: {fl})
	((= outList (EventHandler new:)) add: name: {ol})
	((= backList (EventHandler new:)) add: name: {bl})
	((= newEventHandler (EventHandler new:)) name: {fl2})
	((= newEventHandler_2 (EventHandler new:)) name: {ol2})
	((= newEventHandler_3 (EventHandler new:)) name: {bl2})
	(cast eachElementDo: #perform preSortCode)
	(features eachElementDo: #perform preSortCode)
	(Sort frontList newEventHandler frontSortCode)
	(sortedFeatures add: newEventHandler)
	(Sort outList newEventHandler_2 frontSortCode)
	(sortedFeatures add: newEventHandler_2)
	(sortedFeatures add: regions)
	(sortedFeatures add: locales)
	(Sort backList newEventHandler_3 backSortCode)
	(sortedFeatures add: newEventHandler_3)
...
(instance frontSortCode of Code
	...
	(method (doit param1 &tmp temp0 temp1)
		...
		(if (< (= temp0 (Abs (CosDiv temp1 temp0))) 0)
			(= temp0 32767)
		)
		(return temp0)
	)
