...
(instance rm30 of Room
	...
	(method (doit)
		(cond 
			...
			(warpOut (HandsOff) (= warpOut NULL) (self setScript: teleportOut))
			((and (< (ego y?) 40) (not goingUpstairs))
				(HandsOff)
				(= goingUpstairs TRUE)
				(ego setScript: intoTheTower)
			)
			((and (== (ego edgeHit?) 3) (not leaving)) (HandsOff) (= leaving TRUE) (= warpOut TRUE))
			(
			(and (== (ego onControl: 1) 2) (not forbiddenRoom))
				(HandsOff)
	...
	(method (doVerb theVerb param2)
		(switch theVerb
			(V_CALM
				(if warned (= warpOut TRUE) else (self setScript: warning))
			)
			(V_DAZZLE
				(if warned (= warpOut TRUE) else (self setScript: warning))
			)
...
(instance peacock of Prop
	...
	(method (doVerb theVerb param2)
		(switch theVerb
			(V_LOOK
				(if (> (peacock cel?) 0)
					(messager say: N_PEACOCK V_LOOK 1)
					(showFeathers changeState: 3)
				else
					(messager say: N_PEACOCK V_LOOK 2)
			...
			(V_TALK (messager say: N_PEACOCK V_ALTTALK 0))
			(V_SWORD
				(if warned
					(= warpOut TRUE)
				else
...
(instance dragon of Prop
	...
	(method (doVerb theVerb param2)
		(switch theVerb
			...
			(V_TALK (messager say: N_DRAGON 57))
			(else 
				(super doVerb: theVerb param2 &rest)
			)
		)
...
(instance casket of Prop
	...
	(method (doVerb theVerb param2)
		(switch theVerb
			(V_DO (self setCycle: EndLoop self))
			(else 
				(super doVerb: theVerb param2 &rest)
			)
		)
...
(instance ceiling of Feature
	...
	(method (doVerb theVerb param2)
		(if (== theVerb V_LOOK)
			(messager say: N_CEILING V_LOOK 0)
		else
			(super doVerb: theVerb param2 &rest)
		)
	)
...
(instance bookShelf of Feature
	...
	(method (doVerb theVerb param2)
		(switch theVerb
			(V_LOOK (messager say: N_BOOKSHELF V_LOOK 0 0))
			(V_DO
				(if warned
					(= warpOut TRUE)
				else
...
(instance intro of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if (not (Btst fBeenIn30)) (curRoom doVerb: V_LOOK))
				(ego setScript: 0)
			)
		)
	)
...
(instance teleportOut of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(cond 
					(forbiddenRoom (messager say: N_ROOM 0 C_WARP_OUT 1 self))
					(leaving (messager say: N_ROOM 0 C_WARP_OUT 2 self))
					(else (messager say: N_ROOM 0 C_WARP_OUT 3 self))
				)
			)
			(2
				(Magic
