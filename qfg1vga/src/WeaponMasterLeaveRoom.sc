		...
		(switch (= state newState)
			(0
				...
				((ScriptID rCastleCourtyard 2)
					view: vWeaponsMaster
					setLoop: (if ((ScriptID rCastleCourtyard 2) fightLeft?) 0 else 1)
					cel: 0
				)
				(= cycles 2)
			)
			(1
				(messager say: N_ROOM NULL C_GETTINGBETTER 0 self)
			)
			(2
			...
			(5
				((ScriptID rCastleCourtyard 2)
					setLoop: (if ((ScriptID rCastleCourtyard 2) loop?) 0 else 1)
					cycleSpeed: 6
					moveSpeed: 6
					setCycle: Walk
					setMotion:
						...
						(+ ((ScriptID rCastleCourtyard 2) y?) 20)
						self
				)
				(Bclr fMasterIsHere)
