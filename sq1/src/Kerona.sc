...
(instance keronaRegion of Region
	...
	(method (init &tmp [temp0 3])
		(if
		(or (== currentFloor 1) (!= prevRoomNum spiderRoom))
			(= spiderRoom 0)
		else
		...
		(if (not (ego has: iPlant)) (Load VIEW 13))
		(LoadMany VIEW 15 16)
...
(instance bones of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(verbDo (Print 704 2))
			(verbTaste (Print 704 3))
			(verbTalk (Print 704 4))
			(verbSmell (Print 704 5))
			(else 
...
(instance plant2 of Feature
	...
	(method (init)
		(if (!= currentFloor 1) (self approachVerbs: verbDo))
		(super init: &rest)
	)
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(verbDo
				(cond 
					((ego has: iPlant) (Print 704 6))
					((== (curRoom script?) bigBang) 0)
					(else (curRoom setScript: pickPlant))
				)
			)
			(verbSmell (Print 704 9))
			(verbTaste (Print 704 8))
			(else 
				(super doVerb: theVerb &rest)
			)
...
(instance egoDiesOfThirst of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(ego view: 15 cel: 0 cycleSpeed: 15 setCycle: EndLoop self)
			)
			(3 (= cycles 12))
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				...
				(ego setMotion: 0)
				(Face ego shadow)
				(shadow cel: 2 cycleSpeed: 26 setCycle: EndLoop self)
			)
			(5 (= seconds 3))
			(6
				(shadow dispose:)
