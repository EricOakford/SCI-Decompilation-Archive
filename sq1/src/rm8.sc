...
(instance rm8 of SQRoom
	...
	(method (init)
		...
		(= currentFloor 2)
		(monitor setCycle: Forward init:)
		(dials setCycle: Forward init:)
		(= style (if (== prevRoomNum 9) SCROLLRIGHT else FADEOUT))
		(self
			addObstacle:
				((Polygon new:)
					type: PBarredAccess
		...
		(door
			exiting: (!= prevRoomNum 9)
			light: elevatorLight
			init:
			whereTo: 4
		)
		(if (!= prevRoomNum 4) (HandsOn))
	)
