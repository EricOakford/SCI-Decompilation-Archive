	...
	(method (init)
		(switch prevRoomNum
			...
			(else 
				(= style DISSOLVE)
				(cond 
					((Btst fBoughtDefTechParts)
						(= robotPartsHere TRUE)
						(self setScript: complexRobot)
					)
					((Btst fBoughtSUXDroid)
						(LoadMany SOUND 411 630)
						(self setScript: rightRobot 0 1)
					)
					((and (Btst fBoughtNavDroid) (not (Btst fHaveNavDroid)))
...
(instance blowUpBox of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(ego dispose:)
				(box1 posn: (ego x?) (+ (ego y?) 1) setCycle: EndLoop self)
			)
			(5 (= cycles 6))
			...
			(7
				(EgoDead
					948 0 0
					{Guess this was a bit beyond your capabilities. 
					Speaking of bits, that's about all that remains of you. 
					Hope you can regroup those molecules and get back to the game.}
				)
			)
		)
	)
...
(instance suxRobotS of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				(suxRobot cel: 0 cycleSpeed: 10 setCycle: CycleTo 2 1 self)
				(ego cycleSpeed: 12 setCycle: EndLoop)
			)
			(7
				(soundFx number: 411 loop: 1 play:)
				(suxRobot setCycle: EndLoop self)
			)
			(8
				(suxRobot setLoop: 2 cel: 0 cycleSpeed: 15 setCycle: Forward)
				(= seconds 3)
			)
			(9
			...
			(11
				(EgoDead
					948 0 0
					{You seem to have had trouble maintaining your composure (not to mention your molecular structure). 
					Get yourself together. Unfortunately, you're the universe's only hope.}
				)
			)
		)
	)
...
(instance rightRobot of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(8
				(Face ego (ScriptID ULENCE 1) self)
			)
			(9 (= ticks 12))
			(10
				(theMusic2 number: 622 loop: -1 flags: 1 play:)
				...
				(switch (++ talkCount)
					...
					(1
						(robbieT
							init:
								robBust topGear robMouth
								{What do you want now? I was just about finished with my internal diagnostics check. 
								Now I'm going to have to recalibrate my photo-electric scanner membranes and begin all over again.}
								0 0 1
								self
						)
					)
					(2
						(robbieT
							init:
								robBust topGear robMouth
								{Carbon-based life-forms can be sooo trying sometimes. 
								If you haven't purchased a robot, I have nothing for you. Now, go away!}
								0 0 1
								self
						)
					)
					(3
						(robbieT
							init:
								robBust topGear robMouth
								{(Sigh!) How depressing. Let's begin again from the start and proceed veeerrrryyyy sssslllooowwwllly. 
								I do NOT want to be disturbed. You ARE disturbing me. GO AWAY! Thank you so much.}
								0 0 1
								self
						)
					)
				...
				(switch theItem
					(iKnife
						(robbieT
							init:
								robBust topGear robMouth
								{I am programmed to defend myself against all attacks. Even against such a laughable attempt at destruction as this. 
								It would be a shame to expend the energy required to kill you, don't you agree? Yes, I thought you would.}
								0 0 1
								self
						)
					)
					(iWidget
						(robbieT
							init:
								robBust topGear robMouth
								{Although I am amused at your feeble attempt to modify my magnetic field, I must warn you that I can, 
								and most definitely will, defend myself if need be. Now, please put that thing away. Thank you so much.}
								0 0 1
								self
						)
					)
...
(instance box1 of Prop
	...
	(method (doVerb theVerb)
		(if (OneOf theVerb verbDo verbUse verbTaste)
