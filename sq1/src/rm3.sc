...
(instance rm3 of SQRoom
	...
	(method (init)
		...
		(if (!= (music number?) 355)
			(globalSound number: 353 loop: -1 flags: 1 play: 40)
			(backSound number: 340 loop: -1 play: 30)
			(if (== currentFloor 1)
				(backSound fade: 127 25 10 0)
				(globalSound fade: 127 25 10 0)
			else
				(backSound fade: 50 25 10 0)
				(globalSound fade: 50 25 10 0)
			)
		)
		(droid init:)
		(super init:)
	...
	(method (doit)
		(super doit:)
		(if
		(and (not local0) (& (ego onControl: origin) cBLUE))
			(soundFx number: 311 loop: 1 play:)
			(= local0 1)
			(leftDoor setCycle: EndLoop leftDoor)
		)
		(if
		(and (not local0) (& (ego onControl: origin) cCYAN))
			(soundFx number: 311 loop: 1 play:)
			(= local0 1)
			(rightDoor setCycle: EndLoop rightDoor)
		)
		(if
		(and local0 (not (& (ego onControl: origin) (| cBLUE cCYAN))))
			(soundFx number: 311 loop: 1 play:)
			(= local0 0)
			(if (leftDoor cel?) (leftDoor setCycle: BegLoop leftDoor))
			(if (rightDoor cel?)
	...
	(method (newRoom n)
		...
		(if (!= (music number?) 355)
			...
			(if (and (== n 103) (not (ArcadaCheck 553 16)))
				(globalSound number: 353 fade: 50 25 10 0)
			)
		)
		(super newRoom: n)
	)
		...
		(switch (= state newState)
			...
			(2
				(if (> selfDestructTimer 30)
					...
					(globalSound fade: 30 25 10 0)
					(music fade: 30 25 10 0)
...
(instance goGetCart of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(globalSound number: 302 loop: -1 play:)
				(= droidMoving TRUE)
