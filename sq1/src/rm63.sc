...
(instance rm63 of SQRoom
		...
		(self
			addObstacle:
				((Polygon new:)
					type: PBarredAccess
					init:
						221 78 159 78 210 50 188 50 146 78 55 79
						41 79 0 79 0 0 319 0 319 189 0 189 0 87
						217 87
					yourself:
				)
		)
		(if (== prevRoomNum west) (ego x: 10) (= style 3))
		(panel setCycle: Forward init:)
		(orbs setCycle: Forward setScript: switchit init:)
	...
	(method (doit)
		...
		(cond 
			...
			((& (ego onControl: 0) cBLUE) (self setScript: intoDoor))
		)
	)
)
...
(instance intoDoor of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(ego ignoreActors: TRUE setMotion: PolyPath 195 55 self)
			)
