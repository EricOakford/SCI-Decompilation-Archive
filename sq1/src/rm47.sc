...
(instance rm47 of SQRoom
	...
	(method (init)
		(self
			addObstacle:
				((Polygon new:)
					type: PBarredAccess
					init:
						0 49 213 49 319 137 285 149 292 153 275 164 237 168 163 164 148 158
						133 158 154 176 126 179 121 176 111 177 42 180 0 179
					yourself:
				)
		)
		(ego init:)
		...
		(robot1 init: setCycle: Forward)
		(robot2 init: setCycle: Oscillate)
		(neonhand init: setCycle: Oscillate)
		(lefteye init: setCycle: Oscillate)
		(righteye init: setCycle: Oscillate)
		(pickupArea init:)
		(store init:)
		(bigSign init:)
		(smallSign init:)
		(storeWindow init:)
		(if (and (Btst fHaveNavDroid) (== prevRoomNum 48))
			((ScriptID ULENCE 1) posn: 117 184)
		)
	)
	...
	(method (doit)
		(cond 
			(script 0)
			((& (ego onControl:) cBLUE) (curRoom setScript: intoDBU))
		)
		(super doit:)
	)
)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(if (Btst fHaveNavDroid) ((ScriptID 702 1) posn: 28 179))
			)
			(1
				(HandsOn)
				(self dispose:)
...
(instance radar of Prop
	(properties
		...
		They repel undesirables, such as the grell, which thrive below the sands. It will allow only airborne vehicles in or out.}
		view 147
		loop 2
		cel 4
		priority 2
