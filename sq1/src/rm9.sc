...
(instance rm9 of SQRoom
	...
	(method (init)
		...
		(if (and (== prevRoomNum 10) (Btst fWearingSpacesuit))
...
(instance fireUponEgo of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1 (sarien1 setCycle: EndLoop self))
			(2
				(sarien1 setCycle: BegLoop)
				(if (Btst 16)
					(ego view: 48 setLoop: 0 cel: 0 setCycle: EndLoop self)
				else
					(ego
						view: 6
			...
			(3
				(EgoDead 940 (if (Btst fWearingSpacesuit) 1 else 0) 0 9 0)
				(self dispose:)
			)
		)
	)
...
(instance tv1Script of Script
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			(0 (tv1 setCycle: BegLoop self))
			(1 (= seconds (Random 2 5)))
			(2
				(tv1 setCycle: Forward)
				(= seconds (Random 1 4))
			)
