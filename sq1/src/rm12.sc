...
(instance rm12 of SQRoom
	...
	(method (init)
		...
		(podDoor init: approachVerbs: verbDo verbLook)
		(switch prevRoomNum
			(13
				(podDoor cel: 0)
			...
			(11
				(if (not (Btst fWearingSpacesuit))
					...
					(ego illegalBits: cWHITE posn: 16 157)
					(HandsOn)
				)
			)
			(else 
	...
	(method (doit)
		(cond 
			(script 0)
			((& (ego onControl: origin) cGREEN) (self setScript: fallingScript))
...
(instance fallingScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(ego setPri: (if register 3 else -1) setCycle: EndLoop self)
			)
			(2
...
(instance exitPod of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(soundFx number: 324 loop: 1 play:)
				(podDoor setCycle: EndLoop self)
			)
			(2
				(soundFx stop:)
				(ego show: setPri: 0 cycleSpeed: 12 setCycle: CycleTo 6 1 self)
			)
			(3
...
(instance explode of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0 (= seconds 2))
			(1
				(ego cycleSpeed: 8 setCycle: CycleTo 4 1 self)
			)
			(2
				(soundFx number: 320 loop: 1 play:)
				(ego cycleSpeed: 25 setCycle: CycleTo 7 1 self)
			)
			(3
