...
(class Sq4Narrator of Narrator
	...
	(method (dispose)
		(if (and (> nMsgType -1) (== nMsgType msgType))
			(= msgType oldMsgType)
		)
		(switch tpType
			(1
			...
			(2
				(if returnVal
					(DoDisplay returnVal)
					(if returnVal1
						(DoDisplay returnVal1)
						(if returnVal2
							(DoDisplay returnVal2)
							(if returnVal3 (DoDisplay returnVal3))
						)
					)
				)
			)
	...
	(method (say)
		(if (> nMsgType -1)
			(= oldMsgType msgType)
			(= msgType (if (== oldMsgType 1) 1 else nMsgType))
		)
		(super say: &rest)
	)
	...
	(method (display &tmp [temp0 100])
		(switch tpType
			...
			(2
				(= returnVal (DoDisplay &rest 111))
			)
			(else  (SQ4Print &rest 111))
		)
	)
...
(class Sq4Talker of Talker
	...
	(method (say)
		(if (> nMsgType -1)
			(= msgType
				(if (not (& (= oldMsgType msgType) $0002))
					(& $fffd nMsgType)
				else
					nMsgType
				)
	...
	(method (display &tmp theTalkWidth temp1)
		...
		(SQ4Print &rest 67 theTalkWidth 64 textX textY 111)
	)
	...
	(method (modifyNS &tmp temp0 temp1 temp2)
		...
		(cond 
			((< tStyle 666)
				(self
					nsLeft: (Min (Max 10 nsLeft) (- 319 (+ 10 temp1)))
					nsTop:
						(if (or (== tStyle 5) (== tStyle 4))
							(- (self nsTop?) (/ temp0 2))
						else
							(Min (Max 10 nsTop) (- 189 (+ 10 temp0)))
						)
				)
			)
			(limitToScreen
