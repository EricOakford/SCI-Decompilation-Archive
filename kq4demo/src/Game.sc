...
(class Game of Object
	...
	(method (replay)
		...
		(DrawPic (curRoom curPic?) dpOPEN_TOP)
		(if (!= overlays -1)
			(DrawPic overlays dpOPEN_TOP dpCLEAR)
		)
...
(class Room of Region
	...
	(method (setRegions region &tmp temp0 theScriptNumbers temp2)
		(= temp0 0)
		(while (< temp0 argc)
			(= theScriptNumbers [region temp0])
	...
	(method (setLocales locale &tmp temp0 theScriptNumbers temp2)
		(= temp0 0)
		(while (< temp0 argc)
			(= theScriptNumbers [locale temp0])
			((= temp2 (ScriptID theScriptNumbers))
				number: theScriptNumbers
			)
			(locales add: temp2)
