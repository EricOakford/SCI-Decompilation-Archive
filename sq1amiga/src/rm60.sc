...
(local
	...
	[lpPts 16] = [0 0 319 0 319 173 261 173 270 167 250 167 236 174 0 174]
	[lpPts2 8] = [319 184 319 189 0 189 0 184]
	toX
	toY
)
(instance rm60 of SQRoom
	...
	(method (init)
		...
		(switch prevRoomNum
			...
			(west (= style 12) (ego x: 10))
			(else 
				(= currentFloor 2)
				(= style -32767)
			)
		)
		(if (== currentFloor 2) (LoadMany 128 80))
		(ego init:)
		(super init:)
		(addToPics add: robot eachElementDo: #init doit:)
		(if (== currentFloor 1)
			(robot approachVerbs: 3 2 11 12)
		)
...
(instance shreadEgo of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(ego x: 259 y: 35 loop: 8 cel: 0 setCycle: End self)
			)
			(4
...
(instance shipBlowsUp of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(iris dispose:)
				((ScriptID 703 16) dispose:)
				((ScriptID 703 17) dispose:)
				(ego hide:)
				(e1Door dispose:)
				(curRoom drawPic: 50 7)
				(beam
					view: 328
					loop: 0
					cel: 0
