...
(instance rm48 of Rm
	...
	(method (doit)
		(if (-- local16)
			(switch local17
				(0
					(Palette 6 64 73 (- (Random 0 30) 15))
				)
				(1 (Palette 6 64 73 8))
				(2 (Palette 6 64 73 3))
				(3 (Palette 6 64 73 -3))
	...
	(method (handleEvent event)
		(cond 
			...
			((not (sellRobots register?)) (bugTalker seconds: 0 cue:) (event claimed: 1))
			(
				(and
			...
			(
				(and
					...
					(== (theIconBar curInvIcon?) (Inv at: 10))
		...
		(switch (= state newState)
			...
			(1
				(if register
					...
					(if
					(and (OneOf (++ displayCel) 10 1 4) (Btst 33))
						(++ displayCel)
					)
