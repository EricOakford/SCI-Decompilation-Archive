...
(class InputList of Set
	...
	(method (handleEvent event &tmp thisDistance currentObj inputObj bestAngle
				bestDistance t t2 targetAngle thisAngle workingAngle evX evY curX curY)
		(if (== (event type?) direction)
			...
			(if inputObj
				(SetCursor theCursor TRUE (inputObj cursorX?) (inputObj cursorY?))
			)
		)
	)
