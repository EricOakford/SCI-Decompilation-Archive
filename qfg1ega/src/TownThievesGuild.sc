...
(instance rm332 of Room
	...
	(method (doit)
		(cond 
			...
			(
			(and attackedGuild (not local0) (== (knifeScript state?) 0)) (= local0 1) (localproc_002f))
		)
		(super doit:)
	)
	...
	(method (handleEvent event &tmp silvers itemToFence theInvNum [temp3 60])
		(cond 
			...
			((Said 'look>')
				(cond 
					...
					((Said '[<at,around][/!*,room,building]') (LookAround))
				)
			)
			...
			((Said 'fence,sell>')
				...
				(cond 
					...
					(else
						...
						(HighPrint (Format @temp3 332 38 silvers))
						;"%d silvers.  Here you are."
						(HighPrint (Format @temp3 332 39 [invNum iGold] [invNum iSilver]))
	...
	(method (handleEvent event)
		(switch (event type?)
			(saidEvent
				(if (Said 'ask>')
					(cond 
						((not (ego has: iThiefLicense)) (event claimed: TRUE) (ChiefWontTalk))
						((Said '//dagger,throw')
							(HighPrint 332 40)
						...
						((Said '//game,dagnabit,practice,play,rule') (DagNabItHelp))
						((Said '//bet')
							(HighPrint 332 42)
...
(instance deathScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(EgoDead [deathMessage attackedGuild] #title [deathTitle attackedGuild] #icon vDeathScenes 0 0)
			)
		)
	)
)
