...
(instance rm15 of Room
	...
	(method (handleEvent event &tmp eventType gotIt spell)
		(cls)
		...
		(cond 
			...
			((Said 'get,grab,get/key[<kobold,brass,big,magic,glowing]')
				(cond 
					((cast contains: kobKey)
						...
						(cond 
							((> (ego distanceTo: kobKey) 40)
								(CenterPrint rKoboldCave 5)
							)
							((Btst fKoboldDead)
								(CenterPrint rKoboldCave 6)
								(= gotIt TRUE)
							)
							((Btst fKoboldAwake)
								(CenterPrint rKoboldCave 7)
							)
							((TrySkill STEALTH 35 0)
								(CenterPrint rKoboldCave 8)
								(= gotIt TRUE)
							)
							(else
								(CenterPrint rKoboldCave 9)
								(AwakenKobold)
							)
						)
						(if gotIt
					...
					((ego has: iBrassKey)
						(CenterPrint rKoboldCave 10)
					)
					((ego pickUp: iBrassKey 1)
						(CenterPrint rKoboldCave 11)
					)
					(else
						(CenterPrint rKoboldCave 12)
					)
				)
			)
			(
