...
(procedure (ErasmusPrint moreTime)
	(= theSeconds (+ moreTime 5))
...
(procedure (FenrusPrint moreTime)
	(= theSeconds (+ moreTime 5))
	(Format @str 31 0 &rest)
	(fenrus setScript: 0)
	(cls)
...
(instance rm31 of Room
	...
	(method (doit)
		(cond 
			((and enableErasmusTeaCountdown (> teaTimer 1))
				(-- teaTimer)
			)
			((== teaTimer 1)
				(= teaTimer (Random 150 250))
		...
		(cond 
			...
			((== jokeTimer 1)
				...
				(if
					...
					(self setScript: (ScriptID 135 numJoke))
				)
			)
		)
		(if teleportCued
	...
	(method (handleEvent event)
		(switch (event type?)
			(mouseDown
				(cond 
					((or erasmusTalking fenrusTalking)
						(event claimed: TRUE)
					)
					((MouseClaimed wizBody event shiftDown)
						(LookErasmus)
					)
					((MouseClaimed onRat event shiftDown)
						(LookFenrus)
					)
					((MouseClaimed onLWindow event shiftDown)
						(LookWest)
					)
					(
						(and
					...
					((and (MouseClaimed teacup event shiftDown) (not erasmusDrinking))
						(LookCup)
					)
					((MouseClaimed onCheese event shiftDown)
						(LookCheese)
					)
					((MouseClaimed ego event shiftDown)
						(LookHero)
					)
					((MouseClaimed onCeiling event shiftDown)
						(LookUp)
					)
					((MouseClaimed onStairs event shiftDown)
						(LookDown)
					)
				)
			)
			...
			(saidEvent
				(cond 
					...
					((or (Said 'nap[/!*]') (Said 'go[<to]/nap'))
						(= reasonForLeaving noSleeping)
						(= teleportCued TRUE)
					)
					((Said 'fight,beat,chop,kill,throw')
						(= reasonForLeaving attackedWizard)
						(Bset fAttackedErasmus
							 (= teleportCued TRUE)
						)
					)
					((Said 'cast')
						(= reasonForLeaving castMagic)
						(Bset fAttackedErasmus)
						(= teleportCued TRUE)
					)
					...
					((Said 'look>')
						(cond 
							...
							((or (Said '<down') (Said '/floor,ladder,staircase'))
								(LookDown)
							)
						)
					)
						...
						(if erasmusDrinking
							...
							(cond 
								((or (Said '//open') (Said '//spell<open'))
									(self setScript: (ScriptID 132 0))
								)
								((or (Said '//fetch') (Said '//spell<fetch'))
									(self setScript: (ScriptID 132 1))
								)
								((or (Said '//trigger') (Said '//spell<trigger'))
									(self setScript: (ScriptID 132 2))
								)
								((or (Said '//dazzle') (Said '//spell<dazzle'))
									(self setScript: (ScriptID 132 3))
								)
								((Said '//trap')
									(self setScript: (ScriptID 132 4))
								)
								((Said '//erana')
									(self setScript: (ScriptID 133 0))
								)
								(
									(or
								...
								((Said '//zara')
									(self setScript: (ScriptID 133 2))
								)
								((Said '//baba')
									(self setScript: (ScriptID 133 3))
								)
								((Said '//curse')
									(self setScript: (ScriptID 133 4))
								)
								((Said '//countercurse')
									(self setScript: (ScriptID 133 5))
								)
								((Said '//fenrus,familiar,fenrus')
									(self setScript: (ScriptID 133 6))
								)
								((Said '//enry,enry,ermit,ermit')
									(self setScript: (ScriptID 133 7))
								)
								((Said '//initiate,institute')
									(self setScript: (ScriptID 133 8))
								)
								((Said '//mirror')
									(self setScript: (ScriptID 134 0))
								)
								((Said '//wizard,user,caster,mage,mage')
									(self setScript: (ScriptID 134 1))
								)
								((Said '//nincompoop')
									(self setScript: (ScriptID 134 2))
								)
								((Said '//necromancer')
									(self setScript: (ScriptID 134 3))
								)
								((Said '//magic,technocery')
									(self setScript: (ScriptID 134 4))
								)
								((Said '//game,maze')
									(self setScript: (ScriptID 134 5))
								)
								((Said '//warlock')
									(ErasmusPrint 5 31 23)
								)
								((Said '//spell,scroll')
									(ErasmusPrint 7 31 24)
								)
								((Said '//bandit')
									(ErasmusPrint 9 31 25)
								)
								((Said '//erasmus')
									(ErasmusPrint 4 31 26)
								)
								(else
									(event claimed: TRUE)
									(= talkRet FALSE)
