...
(instance Room67 of Room
	...
	(method (doit)
		...
		(if (and (& (ego onControl: origin) cGREEN) mansionSecretDoorOpen)
			(ego illegalBits: cWHITE)
			(curRoom newRoom: 66)
		)
		(if (and (& (ego onControl:) cMAGENTA) mansionSecretDoorOpen)
			(ego setPri: 8)
		else
			(ego setPri: -1)
		)
			...
			(cond 
				...
				((== (event type?) saidEvent)
					(cond 
						...
						((Said 'look>')
							(cond 
								((and (ego has: iShakespeareBook) (Said '/book'))
									((inventory at: iShakespeareBook) showSelf:)
								)
								...
								((Said '/painting')
									(Print 67 5)
									(= lookedAtStrangeMansionPainting TRUE)
								)
								((Said '/wall<left')
									(cond 
						...
						((or (Said 'get/book') (Said 'get/shakespeare'))
							(cond 
								((ego inRect: 83 119 127 127)
									(Print 67 25)
								)
								((& (ego onControl:) cRED)
									(cond 
										((!= ((inventory at: iShakespeareBook) owner?) 67)
											(Print 67 26)
										)
