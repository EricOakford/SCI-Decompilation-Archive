...
;;;	)
	...
	(method (handleEvent event &tmp keyInvoked evType)
		(event localize:)
		(cond 
			...
			((or
				...
				(if keyInvoked
					(theGame setCursor: (curIcon cursor?) TRUE oldMouseX oldMouseY)
				else
	...
	(method (show &tmp theIcon pnv i theX theY node nextNode obj)
		...
		(PicNotValid pnv)
		; now show bits
		(Graph GShowBits 0 0 height SCRNWIDE VMAP)
	...
	(method (hide &tmp node nextNode obj)
		...
		(Graph GReAnimate 0 0 height SCRNWIDE)
...
;		(if (IsObject event) (event dispose:))
	...
	(method (dispatchEvent event  
		&tmp evtY evtType evtMsg thisIcon evtClaimed thePort
		)
