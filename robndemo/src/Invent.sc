...
;;;	)
	...
	(procedure (DrawInvWindow whom selection 
		...
		(for 	((= node (inventory first:)))
			...
			(cond 
				...
				((obj isMemberOf: IconItem) ;; some other kind of icon
					(++ numIcons)
	...
	(method (doit 
		&tmp thisIcon event evtType evtMsg node newIcon eO keyInvoked
		)
			...
			(cond 
				...
				((& evtType direction)
					(switch (invEvent message?)
						...
						(dirN
							(self retreat: numCols)
						)
						(dirS
							(self advance: numCols)
						)
						(dirStop
							(if (& evtType keyDown)
								(self advanceCurIcon:)
...
;								)
							...
							((not (thisIcon isMemberOf:InvItem))
