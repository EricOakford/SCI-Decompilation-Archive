...
(class IconBar of Set
	...
	(method (handleEvent event &tmp keyInvoked eType)
		(event localize:)
		(cond 
			((& state DISABLED))
			(
	...
	(method (show &tmp theIcon pnv i theX theY node nextNode obj)
		...
		(= port (GetPort))
		(SetPort 0)
		(= underBits (Graph GSaveBits 0 0 height 320 1))
		(= pnv (PicNotValid))
		(PicNotValid 1)
		(= theX 0)
		(= theY 0)
		...
		(PicNotValid pnv)
		(Graph GShowBits 0 0 height 320 1)
	...
	(method (dispatchEvent event &tmp evtY evtType evtMsg thisIcon evtClaimed thePort)
		(= evtY (event y?))
		(= evtType (event type?))
