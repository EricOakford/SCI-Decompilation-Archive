...
(class Inventory of IconBar
	...
	(procedure (DrawInvWindow whom selection &tmp
		...
		(while node
			(cond 
				...
				(else (obj signal: (| (obj signal?) DISABLED)))
			)
	...
	(method (doit &tmp thisIcon event eType [temp3 4] keyInvoked)
		(while
			...
			(cond 
				...
				((& eType direction)
					(switch (event message?)
						...
						(dirStop
							(if (& eType keyDown) (self advanceCurIcon:))
						)
					)
				)
				((== eType keyDown)
