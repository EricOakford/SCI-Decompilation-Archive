...
(class IconBar of Set
	...
	(method (handleEvent event &tmp keyInvoked eType oldCursor oldCurIcon newCursor oldInvIcon newEvent)
		...
		(cond 
			...
			(
				...
				(if keyInvoked
					(theGame
						setCursor: newCursor TRUE oldMouseX oldMouseY
					)
				else
					(theGame
						setCursor:
			...
			(switch evtType
				...
				((| userEvent helpEvent)
					(if (and thisIcon (thisIcon helpStr?))
						(= thePort (GetPort))
						(Format @buffer {%s} (thisIcon helpStr?))
						(Print @buffer)
						(SetPort thePort)
					)
					(if helpIconItem
	...
	(method (noClickHelp &tmp event lastIcon thisIcon [str 100] oldPort eO)
		(= lastIcon (= thisIcon 0))
		(= oldPort (GetPort))
	...
	(method (findIcon theVerb &tmp i thisIcon)
		...
		(while (< i argc)
			(if
			(== ((= thisIcon (self at: i)) message?) theVerb)
				(return thisIcon)
			)
			(++ i)
		)
