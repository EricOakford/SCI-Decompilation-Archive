...
(instance thePlanes of Set
	...
	(method (eachElementDo action &tmp node aCastList obj)
		...
		(for	((= node (FirstNode elements)))
			...
			(for	((= node (FirstNode (aCastList elements?))))
					node
...
;;;	)
	...
	(method (replay &tmp theStyle aPlane aList obj node1 node2 node3)
		...
		(for	((= node1 (FirstNode (planes elements?))))
				node1
				((= node1 (planes nextNode?)))
			(planes nextNode: (NextNode node1))
			(= aPlane (NodeValue node1))
			(AddPlane aPlane)

			; For each cast in the plane's casts list...
			(for	((= node2 (FirstNode ((aPlane casts?) elements?))))
					node2
				...
				(for	((= node3 (FirstNode (aList elements?))))
						node3
		...
		;LSL6-specific
		(global208
			kWindow: (ScrollWindow ScrollCreate global208 (global208 maxItems?))
		)
		(ScrollWindow ScrollReconstruct (global208 kWindow?) global258)
...
;;;#else
		...
		(if (not (ValidPath (KString StrGetData curSaveDir)))
			(= str (String new:))
			(= buf1 (String new:))
...
;;;				(if numCD (self getDisc: numCD) )
				...
				;LSL6-specific
				(if (= temp6 (global208 isVisible?))
					(global208 hide:)
					(= global210 1)
				)
				(= global258 (ScrollWindow ScrollText (global208 kWindow?)))
				(ScrollWindow ScrollDestroy (global208 kWindow?))

	 			(if (not (SaveGame name num (comment data?) (KString StrGetData (version data?))))
					(= buf1 (String new:))
					(= buf2 (String new:))
					...
					(= global176 num)
					(global177 copy: str)
					(global190 copy: curSaveDir)
				)
				(global208
					kWindow: (ScrollWindow 0 global208 (global208 maxItems?))
				)
				(ScrollWindow ScrollReconstruct (global208 kWindow?) global258)
				(ScrollWindow ScrollEnd (global208 kWindow?))
				(if temp6 (global208 show:) (= global210 0))
...
;;;#else
		...
		(if (global208 isVisible?) (global208 hide:))
		(if (!= num -1)
			...
			(if (CheckSaveGame name num (KString 9 (version data?)))
				(ScrollWindow 17 (global208 kWindow?))
				(RestoreGame name num (version data?))
;;;			(if (CheckSaveGame name num (KString StrGetData version))
;;;
...
;;;	)
	...
	(method (reflectPosn theActor theEdge)
		...
		(switch theEdge
			(NORTH
				(theActor y: 188) ;(- (curRoom edgeS?) 1))
			)
			(WEST
