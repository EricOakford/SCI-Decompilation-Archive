...
(instance nClickHelp of Set
	...
	(method (doit &tmp temp0 newEvent theTheCursor)
		(= theTheCursor theCursor)
		(theGame setCursor: helpCursor)
		(while (!= ((= newEvent (Event new:)) type?) 1)
			(newEvent y: (- (newEvent y?) 10))
			(= temp0 (self firstTrue: #onMe newEvent))
			(if (and (talkers size:) (!= local0 temp0))
				(messager cue:)
			)
			(if (and temp0 (!= temp0 local0))
				(Print font: userFont width: 250 modeless: 2)
				(messager
