...
;;;	)
	...
	(method (update theComment names nums &tmp db buf)
		...
		(= numGames (GetSaveFiles (theGame name?) (names data?) (nums data?)))
		(if (== numGames -1)
			(return FALSE)
		)
			...
			(repeat
				...
				(cond
					...
					((== i deleteI)
						...
						(MakeSaveCatName (str data?) (theGame name?))
						((= fd (File new:))
							name: (str data?),
							open:	fTrunc
						)
...
(procedure (HaveSpace)
	(return 	(and 	(< numGames MAXGAMES)
						(CheckFreeSpace (KString StrGetData curSaveDir) CFSEnoughSpaceToSave)
				)
	)
)
