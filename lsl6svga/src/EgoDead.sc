...
(procedure (EgoDead param1 param2 &tmp temp0 theTheCursor temp2 temp3 temp4 temp5 temp6 temp7 temp8 temp9 temp10 temp11 temp12 temp13 temp14 temp15 temp16)
	(= local21 param1)
	(= local0 -1)
	...
	(while (not temp6)
		...
		(= temp6
			(Print
				addButton: 1 2 0 3 1 (+ temp15 1) temp16 82
				addButton: 0 2 0 2 1 (+ temp15 68) temp16 82
				init:
			)
		)
		(DoAudio 3 82 2 0 local21 1)
		(switch temp6
			(0
				(Print back: 7)
...
(instance deathIcon of DCIcon
	...
	(method (init)
		(cond 
			((== local0 -1) ((= cycler (Fwd new:)) init: self))
			((== local0 -2) ((= cycler (CT new:)) init: self 10 1))
			(else ((= cycler (End new:)) init: self cueScript))
		)
		(super init: &rest)
	)
	...
	(method (cycle &tmp theCel)
		...
		(cond 
			...
			(and (!= [local2 cel] -1) (OneOf local21 7 20))
				(cond 
					((not (soundFx handle?)) (soundFx number: [local2 cel] loop: 1 play:))
					((== (soundFx handle?) -1) (soundFx handle: 0) (DoAudio 2 82 2 0 local21 1))
				)
			)
			((!= [local2 cel] -1) (soundFx number: [local2 cel] play:))
		)
		(if
			(and
				(not cycler)
