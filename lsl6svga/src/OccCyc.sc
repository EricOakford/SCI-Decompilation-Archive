...
(class RandOccCycle of OccCyc
	...
	(method (doit &tmp randOccCycleNextCel)
		(= randOccCycleNextCel (self nextCel:))
		(cond 
			(waitCycles (-- waitCycles))
			(
				(and
					(or
						(> randOccCycleNextCel (Random 1 (client lastCel:)))
						(< randOccCycleNextCel 0)
					)
					(< randOccCycleNextCel 0)
				)
				(self cycleDone:)
			)
			(else (client cel: randOccCycleNextCel))
		)
	)
)
